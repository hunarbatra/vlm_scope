#!/usr/bin/env python3
"""
Track SAE feature firing frequencies on an OCR-focused subset of VQAv2.

This mirrors features/spatial/track_firing_vqa_spatial.py but filters the VQAv2
split to questions that contain OCR/reading cues (e.g., "text", "number", "what does it say").

Usage example
-------------
CUDA_VISIBLE_DEVICES=7 python features/ocr/track_firing_vqa_ocr.py \
  --from-layer 0 \
  --to-layer 32 \
  --start-sample 0 \
  --end-sample 20000 \
  --caching-batch-size 8 \
  --output-dir "results/feature_firing_analysis/vqa_ocr" \
...
def _default_ocr_keywords() -> List[str]:
    return [
        # General reading / text recognition
        "text", "word", "words", "letter", "letters", "character", "characters", "string", "strings",
        "print", "printed", "write", "written", "writing", "handwritten", "handwriting", "typed",
        "font", "typeface", "uppercase", "lowercase", "capitalized", "caps", "case",
        "read", "reads", "reading", "readable", "says", "written on", "text on",
        "label", "labeled", "labelled", "caption", "tag", "badge", "sticker", "decal", "marking", "markings",

        # Specific textual entities and codes
        "number", "numbers", "digit", "digits", "alphanumeric", "roman numeral", "roman numerals",
        "code", "codes", "serial", "serial number", "product code", "model number", "part number",
        "barcode", "qr code", "qrcode", "upc", "isbn", "sku",

        # Vehicle/plate identifiers
        "license plate", "plate number", "license number", "registration", "registration number",

        # Time and date
        "time", "clock", "o'clock", "am", "pm", "date", "year", "day", "month",
        "expiry date", "expiration date", "due date", "deadline",

        # Money, prices, percentages
        "price", "cost", "total", "amount", "subtotal", "tax", "tip",
        "dollar", "dollars", "usd", "euro", "euros", "pound", "pounds", "yen", "rupee",
        "cent", "cents", "percent", "percentage", "discount", "sale", "off",

        # Contact / web identifiers
        "phone number", "telephone", "hotline", "email", "e-mail", "website", "web site", "url", "http", "www",
        "hashtag", "handle", "username",

        # Documents and structured text
        "receipt", "invoice", "bill", "ticket", "boarding pass", "schedule", "timetable",
        "menu", "menu item", "nutrition", "nutrition facts", "ingredient", "ingredients", "calorie", "calories", "kcal",

        # Measurements and units often written as text
        "size", "sizes", "small", "medium", "large", "xl", "xxl",
        "weight", "kg", "g", "lb", "lbs", "oz", "ml", "l", "cm", "mm", "inch", "inches", "mph", "km/h", "kph",

        # Signs and signage
        "sign", "signboard", "storefront", "shop sign", "billboard", "poster", "banner",
        "warning", "caution", "stop", "exit", "speed limit", "yield", "one way", "no parking", "open", "closed",

        # Screens and UI text
        "screen", "display", "scoreboard", "score", "timer", "battery", "wifi", "bluetooth", "settings", "button",

        # Address-style strings
        "street", "road", "avenue", "blvd", "boulevard", "drive", "dr", "rd", "st", "apt", "suite",
        "address", "zip", "zipcode", "postal code",

        # Brands and logos
        "name", "brand", "logo", "logo text", "make", "model", "series",

        # Typical OCR-style question phrasings
        "what does it say", "what is written", "what is the text",
        "what is the word", "what is the number", "what number", "what does the sign say",
        "read the text", "read the sign", "spell", "spelled", "spell out",
        "starts with", "ends with", "first letter", "last letter", "contains",
        "how many letters", "how many words", "how many digits",
    ]
...
def _compile_keywords_regex(keywords: List[str]) -> re.Pattern:
    ...
    Multi-word keywords (e.g., "license plate") will be converted to allow flexible spacing
    or a hyphen (e.g., "license plate" or "license-plate").
    """
    escaped_variants = []
    for kw in keywords:
        kw = kw.strip()
...
class OCRVQADataset:
    """VQAv2 validation subset filtered to OCR questions by keyword/regex.
    ...
    def __init__(
        ...
        if keywords_file is not None and os.path.exists(keywords_file):
            ...
            keywords_list = _default_ocr_keywords()
        ...
        print(
            f"[INFO] OCRVQADataset: kept {len(self._indices)} of {len(self._base)} questions ({src})"
        )
...
def track_feature_firing_chunk(
    ...
    Mirrors features/spatial/track_firing_vqa_spatial.py for consistency so results are
    ...
    try:
        wandb_run = wandb.init(project="sae-feature-firing", name=f"vqa_ocr_feature_firing_{start_idx}_{end_idx}")
...
def main():
    parser = argparse.ArgumentParser(description="Track SAE feature firing on OCR-focused VQAv2 subset")
    ...
    parser.add_argument("--start-sample", type=int, default=0, help="Starting sample index (in OCR subset)")
    parser.add_argument("--end-sample", type=int, default=50000, help="Ending sample index (in OCR subset)")
    parser.add_argument("--caching-batch-size", type=int, default=16, help="Batch size for processing")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/feature_firing_analysis/vqa_ocr",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to newline-separated OCR keywords/phrases",
    )
    parser.add_argument(
        "--vqa-cache-dir",
        type=str,
        default=".cache/vqa_ocr_filter",
    ...
    print(f"[INFO] OCR VQA analysis for samples {args.start_sample} → {args.end_sample}")
    ...
    # Load OCR VQA dataset using OCRVQADataset class
    ...
    print(
        f"[INFO] OCR filter kept {len(dataset)} samples. Processing range: "
    ...
    print(f"[INFO] OCR VQA analysis complete! Results saved to {args.output_dir}")
