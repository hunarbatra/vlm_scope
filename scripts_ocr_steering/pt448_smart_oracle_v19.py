#!/usr/bin/env python3
"""
Smart Oracle v19 — 24 per-relation alpha overrides (+1 new), 0 feature reassignments.

Changes from v18:
- "enclosed by" alpha override → 1.25 (L12_F2257)
  [best α=1.25 → +19.05% vs oracle α=0.75 → +9.52%, +9.53pp, N=21, extended_alpha_sweep confirmed]
- All v18 feature assignments and overrides retained

Total: 24 overrides (+1 new), same feature map as v18

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v19/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v19.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v19")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "enclosed by":    1.25, # +19.05% vs oracle +9.52%, +9.53pp, N=21 (NEW v19)
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v19_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v18 expected ~+5.22%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v19 — 24 per-relation alpha overrides (+1 new)", flush=True)
    ...
    print(f"\n[RESULT v19] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v18 expected ~+5.22%;  v17 ~+5.20%;  v16 ~+5.15%;  v15 ~+5.10%;  v14 ~+5.00%]", flush=True)
    ...
    res = {"version": "v19", "alpha_mode": "per_relation_alpha_24overrides_v19",
