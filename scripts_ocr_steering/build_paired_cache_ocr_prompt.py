#!/usr/bin/env python3
"""
Paired contrast cache with the "ocr" prompt (pt-448's native OCR mode).

Per-sample:
  1. Run mix-448 with prompt "ocr" (no question). Get response.
  2. is_correct (LENIENT, OCR-Bench official) = (gt in resp or resp in gt).
  3. neg_answer:
       if mix correct  -> synthetic char-substitution distortion of GT
       if mix incorrect -> mix's actual ocr response (real OCR error pattern)
  4. Forward "ocr\\n{GT}"        -> pos hiddens (mean over text tokens)
  5. Forward "ocr\\n{neg_answer}" -> neg hiddens (mean over text tokens)

Output:
  /data1/vlm_scope_sae_mix448_textonly/analysis_ocr/paired_cache_ocrprompt/vi_{si:05d}.pt

Usage:
  CUDA_VISIBLE_DEVICES=1 python3 -B scripts/build_paired_cache_ocr_prompt.py
"""
...
TRAIN_END      = 1000
NUM_LAYERS     = 26
MAX_NEW_TOKENS = 256   # ocr mode can produce long transcripts
...
def main():
    ...
    print("=" * 80)
    print("PAIRED CONTRAST CACHE — OCR-Bench, mix-448, prompt='ocr'")
    print("=" * 80, flush=True)

    print("[INFO] Loading OCR-Bench...", flush=True)
    ds = load_dataset("echo840/OCRBench", split="test")
    print(f"  N={len(ds)}, building cache for indices 0..{TRAIN_END-1}", flush=True)
    ...
    for si in range(TRAIN_END):
        ...
        try:
            ...
            # 1) mix-448 generate with "ocr" prompt
            ...
            is_correct = _correct_lenient(resp, gt)
            ...
            if (n_built + n_skip) % 25 == 0:
                ...
                print(f"  [si={si:4d}] built={n_built} skip={n_skip} err={n_err}  "
                      ...
                      f"resp='{resp_short}' gt='{gt[:25]}' correct={is_correct}",
                      flush=True)
