#!/usr/bin/env python3
"""
Smart Oracle v18 — 23 per-relation alpha overrides (same count), 2 feature reassignments.

Changes from v17:
- "within" feature reassigned L15_F220 → L12_F2257
  [L12 oracle α=0.75 → +6.06% vs L15 oracle +3.03%, +3.03pp, N=33, reassignment_sweep confirmed]
- "outside" feature reassigned L12_F2257 → L15_F220 + alpha override α=1.0
  [L15 α=1.0 → +12.50% vs L12 α=1.0 → +9.38%, +3.12pp, N=32, reassignment_sweep confirmed]
- "facing" KEPT at L12_F2257 — oracle α=0.75 → +8.82% is already best
  [L12 oracle α=0.75 → +8.82% beats L9_F387 best α=0.8 → +7.84%, N=306]
- All v17 overrides retained

Total: 23 overrides (unchanged count, 2 feature reassignments)

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v18/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v18.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v18")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "outside":     1.0,    # +12.50% vs oracle +3.12%, +9.38pp, N=32 (NEW v18, feat reassigned)
    # L6/F7539 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v18_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v17 expected ~+5.20%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v18 — 23 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v18] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v17 expected ~+5.20%;  v16 ~+5.15%;  v15 ~+5.10%;  v14 ~+5.00%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v18", "alpha_mode": "per_relation_alpha_23overrides_v18",
