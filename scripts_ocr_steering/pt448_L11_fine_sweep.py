#!/usr/bin/env python3
"""
Fine alpha sweep for L11/F12278 on full VSR.
L11/F12278 hit +2.34% at α=0.5 - new record (surpassing L15 +2.09%).
Sweep finer alphas to find true peak.
"""
...
N_LAYERS    = 26
LAYER, FEATURE, START_LAYER = 11, 12278, 5
CAA_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_caa_steering/caa_vectors")
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L11_fine_sweep")
...
ALPHAS = [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.9, 1.0, 1.25, 1.5, 2.0]
...
def main():
    ...
    result_path = OUT_DIR / "L11_F12278_fine_sweep.json"
    ...
    vsr_all = concatenate_datasets([load_dataset(VSR_DATASET, data_files=data_files, split=s) for s in ["train","dev","test"]])
    indices = list(range(len(vsr_all)))

    print("[INFO] Loading pt-448...", flush=True)
    processor_pt = AutoProcessor.from_pretrained(MODEL_PT)
    model_pt = PaliGemmaForConditionalGeneration.from_pretrained(MODEL_PT, dtype=torch.bfloat16).to(device).eval()
    ...
    vecs = {int(l): (ld["v_caa_norm"]/ld["v_caa_norm"].norm().clamp(min=1e-8)).to(dtype).to(device)
            for l, ld in saved["caa_data"].items() if ld.get("v_caa_norm") is not None}
    ...
    for vi in indices:
        ...
        try:
            iids, attn, pv = process_vlm_inputs(img, _build_vsr_prompt(str(ex.get("caption",""))), processor_pt, model_pt, device=device)
            ...
            try:
                ...
                with nns_pt.trace(input_ids=iids, attention_mask=attn, pixel_values=pv):
                    for l in range(START_LAYER, N_LAYERS):
                        ...
                        v_l = vecs[l]; v_col = v_l.unsqueeze(1)
    ...
    result = {"layer": LAYER, "feature": FEATURE, "start_layer": START_LAYER,
