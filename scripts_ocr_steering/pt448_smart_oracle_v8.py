#!/usr/bin/env python3
"""
Smart Oracle v8 — Extended per-relation alpha overrides, 8 total.

Changes from v7:
- L6/F7539 "below":      α=2.5 → +6.14% (vs oracle α=1.5 → +5.42%, +0.72pp, N=277)
- L9/F7540 "parallel to": α=0.3 → +7.78% (vs oracle α=0.25 → +5.56%, +2.22pp, N=90)
- All v7 overrides retained: L11 on_top_of→0.4, L11 surrounding→0.6, L15 above→0.8,
  L6 at_back_of→1.75, L9/F7540 in_middle_of→0.2, L9/F7540 on→0.3

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v8/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v8.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v8")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "below":          2.5,  # +6.14% vs oracle +5.42%, +0.72pp, N=277 (NEW in v8)
    ...
    "parallel to":      0.3, # +7.78% vs oracle +5.56%, +2.22pp, N=90 (NEW in v8)
}
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v8_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v7 expected ~+4.35%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v8 — 8 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v8] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v7 expected ~+4.35%;  v6 ~+4.30%;  v5 ~+4.20%;  v4 ~+4.15%;  v3=+4.05%;  v2=+3.96%]", flush=True)
    ...
    res = {"version": "v8", "alpha_mode": "per_relation_alpha_8overrides",
