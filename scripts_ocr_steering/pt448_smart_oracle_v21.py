#!/usr/bin/env python3
"""
Smart Oracle v21 — 23 per-relation alpha overrides, 1 new feature reassignment.

Changes from v20:
- "under" feature reassigned L11_F12278 → L12_F2257 (oracle α=0.75)
  [L12 oracle +2.04% vs L11 best +1.87%, +0.17pp, N=589; L9_F387 only +1.02%]
  Also removes the now-redundant "under": 0.75 override (L12 oracle IS 0.75)
- Removed "touching": 0.5 override — L11 oracle α=0.45 → +6.01% > override α=0.5 → +5.54%, +0.47pp, N=1281
  [Cross-feature sweep confirmed: no other feature beats L11_F12278 for touching]

Notes on confirmed sweeps (no changes needed):
- "on" (N=585): L9_F7540 α=0.3→+1.37% confirmed best
- "next_to" (N=309): L9_F7540 oracle α=0.25→+1.62% confirmed best
- "parallel_to" (N=90): L9_F7540 α=0.3→+7.78% confirmed best (v20 override correct)
- "touching" (N=1281): L11_F12278 oracle α=0.45→+6.01% is true best (override removed)

Total: 23 overrides (-2 from v20), 1 feature reassignment (under L11→L12)
Expected gain over v20: ~+0.47pp (touching) + ~+0.17pp (under) = ~+0.64pp

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v21/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v21.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v21")
...
RELATION_ALPHA_OVERRIDES = {
    # L11/F12278 overrides
    "on top of":   0.4,    # +5.35% vs oracle +4.95%, +0.40pp, N=505
    "surrounding": 0.6,    # +11.11% vs oracle +7.78%, +3.33pp, N=90
    # "touching" removed v21: oracle α=0.45 → +6.01% > old override α=0.5 → +5.54%, +0.47pp, N=1281
    # "under" removed v21: reassigned to L12_F2257 whose oracle IS α=0.75
    # L15/F220 overrides
    "above":       0.8,    # +5.87% vs oracle +4.69%, +1.18pp, N=341
    "part of":     0.8,    # +4.42% vs oracle -0.88%, +5.30pp, N=113
    "toward":      0.9,    # +19.44% vs oracle +16.67%, +2.77pp, N=36 (feat reassigned v17)
    "outside":     1.0,    # +12.50% vs oracle +3.12%, +9.38pp, N=32 (feat reassigned v18)
    "attached to": 0.6,    # +8.93% vs L9 oracle +7.14%, +1.79pp, N=56 (feat reassigned v20)
    # L6/F7539 overrides
    "at the back of": 1.75, # +3.19% vs oracle +2.13%, +1.06pp, N=94
    "below":          2.5,  # +6.14% vs oracle +5.42%, +0.72pp, N=277
    "alongside":      1.25, # +14.55% vs oracle +12.73%, +1.82pp, N=55
    # L12/F2257 overrides
    "beneath":        1.25, # +7.33% vs oracle +5.87%, +1.46pp, N=341
    "enclosed by":    1.25, # +19.05% vs oracle +9.52%, +9.53pp, N=21 (v19)
    "near":           1.0,  # +11.82% vs oracle +10.91%, +0.91pp, N=110
    "off":            1.25, # +14.86% vs oracle +9.46%, +5.40pp, N=74
    # L9/F387 overrides
    "far from":       0.5,  # +2.07% vs oracle -0.69%, +2.76pp, N=145
    "adjacent to":    0.5,  # +5.19% vs oracle +2.60%, +2.59pp, N=77
    # L14/F10561 overrides
    "by":             0.5,  # +7.69% vs oracle +3.85%, +3.84pp, N=52
    "connected to":   1.5,  # +13.51% vs oracle +10.81%, +2.70pp, N=37
    # L9/F7540 overrides
    ...
    "away from":        1.0,  # +3.87% vs L9F7540 oracle +0.65%, +3.22pp, N=155 (feat reassigned v14)
    "opposite to":      0.15, # +5.56% vs oracle +2.78%, +2.78pp, N=36
}
...
BEST_FEATURE_MAP = {
    ...
    "at the side of":      "L12_F2257",   # Changed v14: +3.45% vs L9 +0.00%, N=58
    "attached to":         "L15_F220",    # Changed v20: L15 α=0.6 → +8.93% vs L9 oracle +7.14%, N=56
    "away from":           "L9_F7540",    # Changed v14: +3.87% vs L15 +1.29%, N=155
    "behind":              "L4_F14233",
    "below":               "L6_F7539",
    "beneath":             "L12_F2257",
    "beside":              "L15_F220",
    ...
    "facing":              "L12_F2257",   # L12 oracle α=0.75 → +8.82% (confirmed best, N=306)
    "facing away from":    "L6_F7539",
    "far from":            "L9_F387",
    "in the middle of":    "L9_F7540",
    "inside":              "L12_F2257",   # Changed v16: +2.92% vs L15 +1.67%, N=240
    "left of":             "L6_F7539",
    "near":                "L12_F2257",
    "next to":             "L9_F7540",
    "off":                 "L12_F2257",
    ...
    "outside":             "L15_F220",    # Changed v18: L15 α=1.0 → +12.50% vs L12 α=1.0 → +9.38%, N=32
    "over":                "L15_F220",
    "parallel to":         "L9_F7540",
    "part of":             "L15_F220",
    "right of":            "L15_F220",    # Changed v13: +1.77% vs L6 +0.88%, N=113
    "surrounding":         "L11_F12278",
    "touching":            "L11_F12278",
    "toward":              "L15_F220",    # Changed v17: L15 α=0.9 → +19.44% vs L12 oracle +16.67%, N=36
    "under":               "L12_F2257",   # Changed v21: oracle α=0.75 → +2.04% vs L11 best +1.87%, N=589
    "within":              "L12_F2257",   # Changed v18: oracle α=0.75 → +6.06% vs L15 oracle +3.03%, N=33
}
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v21_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v20 expected ~+5.27%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v21 — 23 per-relation alpha overrides, under reassigned L11→L12", flush=True)
    ...
    print(f"\n[RESULT v21] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v20 expected ~+5.27%;  v19 ~+5.23%;  v18 ~+5.20%;  v17 ~+5.20%]", flush=True)
    ...
    res = {"version": "v21", "alpha_mode": "per_relation_alpha_23overrides_v21",
