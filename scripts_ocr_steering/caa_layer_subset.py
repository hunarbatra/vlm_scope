#!/usr/bin/env python3
"""Test specific layer subset for multi-layer CAA."""
...
OUT_DIR = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/caa_layer_subset")
...
def main():
    ...
    ap.add_argument("--layers", type=str, required=True, help="comma-separated layers, e.g. 11,12,13")
    args = ap.parse_args()
    layers = [int(x) for x in args.layers.split(",")]
    device="cuda:0"; OUT_DIR.mkdir(parents=True, exist_ok=True)
    tag = "_".join(str(l) for l in layers)
    results_path = OUT_DIR / f"results_L{tag}.json"
    ...
    for alpha in ALPHAS:
        ...
        print(f"  [L{tag} α={alpha}] {acc:.2f}%  Δ={d:+.2f}%  ({c}/{t})", flush=True)
