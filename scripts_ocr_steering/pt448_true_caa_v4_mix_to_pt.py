#!/usr/bin/env python3
"""
True CAA mix-src → steer pt-448.

Uses cached mix-448 mean-over-text-tokens hidden states (mix_hidden/*.pt) to
compute label-contrastive vectors, then steers pt-448 on each R(F) subset.

Two conditions per feature F at layer lF:
  MIDDLE  v[13] from all ~10,972 VSR samples at L13 (mix hidden cache).
  FEATURE v[lF] from R(F) ∩ {SAE feature F fires} at lF (mix hidden cache).

This is the cache-based analogue of v3 (mix→pt), using mean-over-text-tokens
instead of the per-layer feature vectors that v3 used. Directly comparable to
pt448_true_caa_v4_pt_src.py (pt→pt with same extraction protocol).

Usage:
    CUDA_VISIBLE_DEVICES=3 python3 -B pt448_true_caa_v4_mix_to_pt.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_true_caa_v4_mix_to_pt")
...
def compute_vectors(vsr_all):
    print("[STEP 1] Computing True CAA vectors from mix-448 hidden cache...", flush=True)
...
def main():
    ...
    print("=" * 70)
    print("True CAA mix-src → steer pt-448")
    print(f"MIDDLE: L{MIDDLE_LAYER}, all VSR; FEATURE: lF, R(F)∩fire_F")
    print("Cache: mix_hidden (mean-over-text-tokens)")
    print("=" * 70, flush=True)
    ...
    # Compute vectors from mix-448 cache (no live passes needed)
