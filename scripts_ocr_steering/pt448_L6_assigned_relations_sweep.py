#!/usr/bin/env python3
"""
Fine alpha sweep for L6/F7539 on all oracle-assigned relations.

L6/F7539 oracle alpha is 1.5 (from own-relation subset sweep on 'left of'/'right of').
This script tests all oracle-assigned relations for L6/F7539 to find
per-relation optimal alphas.

Oracle-assigned relations for L6/F7539:
  across_from, alongside, at_back_of, below, facing_away_from, left_of, right_of
...
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_L6_assigned_relations_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L6_assigned_sweep")
...
# Relations currently assigned to L6/F7539 in the oracle
TARGET_RELATIONS = [
    "across from",
    "alongside",
    "at the back of",
...
def main():
    ...
    # Summary table
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':22s}  {'N':5s}  {'Base':7s}  {'L6(oracle)':10s}  {'L6(best)':8s}  {'L12_best':8s}  {'L15_best':8s}", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        ...
        d6_ora  = f6.get("oracle_delta", 0) or 0
        d6_best = f6.get("best_delta", 0) or 0
        d12 = f12.get("best_delta", 0) or 0
        d15 = f15.get("best_delta", 0) or 0
        print(f"{rel:22s}  {r.get('n',0):5d}  {base:7.2f}%  {d6_ora:+10.2f}%  {d6_best:+8.2f}%  {d12:+8.2f}%  {d15:+8.2f}%", flush=True)
