#!/usr/bin/env python3
"""
True CAA v4b — answer-appended token[-2] extraction, targeting pt-448.

Identical extraction protocol to pt448_true_caa_v4b.py (Rimsky et al. 2023 /
Meg Tong convention), but SOURCE and TARGET are both pt-448 instead of mix-448.
...
Exact match to Rimsky et al. 2023 / Meg Tong's implementation:
  ...
  - Extract hidden state at token index -2 (second-to-last) from pt-448.
  - Compute label-contrastive vector: mean(h[-2]|label=1) − mean(h[-2]|label=0)

Source = Target = pt-448 (v4b protocol). Two conditions: MIDDLE (L13) and FEATURE (lF).

Fills the missing cell in the v4b 2×2 grid:
  v4b mix→mix: done (pt448_true_caa_v4b.py)
  v4b pt→pt:   THIS SCRIPT

Usage:
    CUDA_VISIBLE_DEVICES=1 python3 -B pt448_true_caa_v4b_pt_src.py
"""
...
OUT_DIR      = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_true_caa_v4b_pt_src")
...
def extract_last_token_hidden(model, processor, vsr_all, vis, layers, device,
                              label_map=None):
    """
    Run pt-448 forward on each sample in vis, capture token[-2] hidden state.
    Answer is appended so token[-2] = ':' after "Answer" (last prompt token).
    """
    ...
    for idx, vi in enumerate(vis):
        ...
        def make_hook(l):
            def hook_fn(module, inp, out):
                ...
                captures[l] = h[0, -2, :].detach().float().cpu()
            return hook_fn
...
def compute_vectors(model, processor, vsr_all, device):
    ...
    print("\n[STEP 2] Extracting token[-2] hidden states (answer-appended) from pt-448...", flush=True)
    layer_hidden = {}
...
def main():
    ...
    # Compute vectors via live forward passes on pt-448
