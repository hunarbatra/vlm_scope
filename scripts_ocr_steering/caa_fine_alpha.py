#!/usr/bin/env python3
"""
Fine-grained α sweep around the peak:
- MEANPOOL L13 single layer, α ∈ {2.5, 3.5, 4.0, 4.5, 6, 7}
- MEANPOOL all-layers, α ∈ {0.75, 1.5, 2.5, 3.5}

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B caa_fine_alpha.py [--config single|multi]
"""
...
OUT_DIR = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/caa_fine_alpha")
...
def main():
    ...
    results_path = OUT_DIR / f"results_{args.config}.json"

    if args.config == "single":
        layers = [13]
        alphas = [2.5, 3.5, 4.0, 4.5, 6.0, 7.0]
    else:
        layers = LAYERS
        alphas = [0.75, 1.25, 1.5, 1.75, 2.5]
    ...
    for l in layers:
        ...
        if v is not None:
            vecs[l] = v / v.norm().clamp(min=1e-8)  # unit-normed
    print(f"[INFO] Fine α sweep {args.config} layers={layers} α={alphas}", flush=True)
    gc.collect()
    ...
    for alpha in alphas:
        ...
        print(f"  [α={alpha}] {args.config}: {acc:.2f}%  Δ={d:+.2f}%  ({c}/{t})", flush=True)
