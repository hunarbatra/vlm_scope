...
def _firing_worker(gpu_id, layer_indices):
    ...
    vsr_dir = ANALYSIS_DIR / "firing_vsr_pertoken"
    ...
    if remaining_vsr:
        ...
        for layer_idx in remaining_vsr:
            ...
            print(f"[Firing GPU{gpu_id}] VSR L{layer_idx}: {n_samples} samples, {total_text_tokens} tokens ({n_failed} failed), "
                  f"features firing >1%: {(token_fire_count > total_text_tokens * 0.01).sum()}", flush=True)
...
def step6_spatial():
    ...
    vqa_dir = ANALYSIS_DIR / "firing_vqa_pertoken"
    vsr_dir = ANALYSIS_DIR / "firing_vsr_pertoken"
    out_dir = ANALYSIS_DIR / "spatial_pertoken"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for layer_idx in range(N_LAYERS):
        ...
        n_vsr = vsr_data["n_tokens"]
        fire_vsr = np.array(vsr_data["fire_count"])

        if n_vqa == 0 or n_vsr == 0:
            continue

        print(f"  Layer {layer_idx}: n_vqa_tokens={n_vqa}, n_vsr_tokens={n_vsr}")
    ...
    spatial.to_csv(out_dir / "spatial_features_pertoken.csv", index=False)
    ...
    summary = {
        ...
        "method": "VQA vs VSR per-token (matching original LLaVA-MORE pipeline)",
    }
...
def step7_lexical(n_gpus=N_GPUS):
    ...
    # Load spatial candidates from step 6 (per-token results)
    csv_path = ANALYSIS_DIR / "spatial_pertoken" / "spatial_features_pertoken.csv"
    if not csv_path.exists():
...
def step8_intersection():
    ...
    # Load spatial (per-token results)
