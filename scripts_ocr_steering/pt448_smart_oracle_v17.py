#!/usr/bin/env python3
"""
Smart Oracle v17 — 23 per-relation alpha overrides (+1 new).

Changes from v16:
- "toward" feature reassigned L12_F2257 → L15_F220 + alpha override α=0.9
  [L15 α=0.9 → +19.44% vs L12 oracle α=0.75 → +16.67%, +2.77pp, N=36, reassignment_sweep confirmed]
- All v16 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v17/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v17.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v17")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "toward":      0.9,    # +19.44% vs oracle +16.67%, +2.77pp, N=36 (NEW v17, feat reassigned)
    # L6/F7539 overrides
    ...
    "outside":        1.0,  # +9.38% vs oracle +3.12%, +6.26pp, N=32
    # L9/F387 overrides
    ...
    "away from":        1.0,  # +3.87% vs L9F7540 oracle +0.65%, +3.22pp, N=155 (feat reassigned)
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v17_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v16 expected ~+5.15%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v17 — 23 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v17] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v16 expected ~+5.15%;  v15 ~+5.10%;  v14 ~+5.00%;  v13 ~+4.90%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v17", "alpha_mode": "per_relation_alpha_23overrides",
