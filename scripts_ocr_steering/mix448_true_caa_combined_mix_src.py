#!/usr/bin/env python3
"""
True CAA combined steering — mix→mix (mix-448 MIDDLE vectors → steer mix-448).

Uses mix-448 cache MIDDLE vector at L13 (global label-contrastive direction).
Tests: GLOBAL MIDDLE on union10 (N=4882) and top3_union (N=1413).
Completes the 2x2 grid: mix→mix cell (vs mix→pt done: +2.83%/+5.87%).

Expected: weaker than mix→pt given mix high baseline (~76%), ceiling effect.

Usage:
    CUDA_VISIBLE_DEVICES=7 python3 -B mix448_true_caa_combined_mix_src.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/mix448_true_caa_combined_mix_src")
...
def main():
    ...
    print("=" * 70)
    print("True CAA Combined Steering — mix→mix (MIDDLE vectors summed)")
    ...
    for eval_name, vis_list, labels_list in [
        ...
        if base_key not in all_results:
            ...
            print(f"\n[{eval_name}] mix-448 baseline (n={len(vis_list)})...", flush=True)
            bc = bt = 0
    ...
    # Summary
    print(f"\n{'='*70}")
    print("True CAA Combined — mix→mix")
    print(f"{'='*70}")
    for rkey, label in [
