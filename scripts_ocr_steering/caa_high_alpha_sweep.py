#!/usr/bin/env python3
"""
High-α CAA sweep — mix→pt L13 MEANPOOL, α ∈ {20, 30, 50, 80, 120, 200}.

Motivation: our CAA vector raw norm is only ~3% of hidden-state norm. At
unit-normalized α=20 we inject only ~9% of hidden magnitude. Rimsky's working
regime is ~30-50% perturbation. Need higher α.

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B caa_high_alpha_sweep.py
"""
...
OUT_DIR = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/caa_high_alpha_sweep")
...
TRAIN_END = 8777
LAYER = 13
ALPHAS = [30.0, 50.0, 80.0, 120.0, 200.0]
...
def main():
    ...
    vec = pos/pn - neg/nn
    print(f"  CAA L13: norm={vec.norm():.3f}  pn={pn} nn={nn}", flush=True)
    ...
    for alpha in ALPHAS:
        ...
        print(f"  [α={alpha}] HIGH-α L13: {acc:.2f}%  Δ={d:+.2f}%  ({c}/{t})", flush=True)
