#!/usr/bin/env python3
"""
True CAA pt-src — pt-448 hidden states as steering source, pt-448 as target.

Uses cached pt-448 last-token hidden states (pt_hidden/*.pt) to compute
label-contrastive vectors, then steers pt-448 on each R(F) subset.

Two conditions per feature F at layer lF:
  MIDDLE  v[13] from all ~10,972 VSR samples at L13. Cache read only.
  FEATURE v[lF] from R(F) ∩ {SAE feature F fires} at lF. Cache read only.

This is the pt→pt analogue of v4 (mix→mix). Completes the 2×2 grid:
  mix→mix  (v4)           pt→pt  (this script)
  mix→pt   (v3/middle)    pt→mix (future)

Cache: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_hidden_delta/pt_hidden/
  vi_NNNNN.pt: {layer: tensor([2304], bfloat16)} — mean-over-text-tokens hidden.

NOTE: The pt_hidden cache stores mean-over-text-tokens (not last token), so
these vectors differ from v4/v4b (last-token extraction). This is a different
extraction protocol but allows cache-only operation without live forward passes
(much faster — no 10,972-sample extraction pass needed).

Usage:
    CUDA_VISIBLE_DEVICES=2 python3 -B pt448_true_caa_v4_pt_src.py
"""
...
OUT_DIR       = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_true_caa_v4_pt_src")
...
def main():
    ...
    print("=" * 70)
    print("True CAA pt-src — pt-448 hidden states → steer pt-448")
    ...
    # Compute vectors from pt-448 cache (no live passes needed)
