#!/usr/bin/env python3
"""
Smart Oracle v11 — Extended per-relation alpha overrides, 13 total.

Changes from v10:
- L12/F2257 "off": α=1.25 → +14.86% (vs oracle α=0.75 → +9.46%, +5.40pp, N=74)
- L12/F2257 "near": α=1.0 → +11.82% (vs oracle α=0.75 → +10.91%, +0.91pp, N=110)
- L11/F12278 "touching": α=0.5 → +5.54% (vs oracle α=0.45 → +5.01%, +0.53pp, N=1281)
- All v10 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v11/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v11.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v11")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "touching":    0.5,    # +5.54% vs oracle +5.01%, +0.53pp, N=1281 (NEW in v11)
    ...
    "near":           1.0,  # +11.82% vs oracle +10.91%, +0.91pp, N=110 (NEW in v11)
    "off":            1.25, # +14.86% vs oracle +9.46%, +5.40pp, N=74 (NEW in v11)
    # L9/F7540 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v11_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v10 expected ~+4.60%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v11 — 13 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v11] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v10 expected ~+4.60%;  v9 ~+4.55%;  v8 ~+4.45%;  v7 ~+4.35%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v11", "alpha_mode": "per_relation_alpha_13overrides",
