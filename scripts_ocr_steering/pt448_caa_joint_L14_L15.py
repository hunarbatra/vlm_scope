#!/usr/bin/env python3
"""
Joint alpha × start-layer sweep for L14/F10561 and L15/F220.

Known best results (caa_sae_down, default start=SAE layer):
  L14/F10561: caa_sae_down α=2.0 → +10.75%  (also caa_all_ml α=10 → +10.75%)
  L15/F220:   caa_sae_down α=0.75 → +7.96%

This script sweeps the 2D (alpha × start_layer) space to find the true joint optimum.

L14: alpha=[0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0] × start=[0,1,2,3,5,8,10,12,14]
L15: alpha=[0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0] × start=[0,1,2,3,5,8,10,12,15]

Usage:
    CUDA_VISIBLE_DEVICES=5 python3 pt448_caa_joint_L14_L15.py   (GPU5)
    CUDA_VISIBLE_DEVICES=7 python3 pt448_caa_joint_L14_L15.py   (GPU7, L15 only via env var)
...
# Which features to run — set GPU_FEATURES env var to "L14" or "L15" or leave blank for both
_RUN = os.environ.get("GPU_FEATURES", "BOTH")

FEATURES_ALL = [
    (14, 10561, ["close to"],                            +10.75),
    (15, 220,   ["across from", "at the left side of"], +7.96),
]
...
def main():
    ...
    print(f"\n{'='*80}")
    print("Joint Alpha×Start Sweep (L14+L15) — Summary")
    print(f"{'='*80}")
