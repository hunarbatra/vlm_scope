#!/usr/bin/env python3
"""
pt-448 VSR feature steering experiment.
Tests whether amplifying top-N spatial features (by pt-448 delta_vsr) in the
pretrained model can recover VSR accuracy toward the fine-tuned mix-448 baseline.

For each top-N feature (by pt-448 delta_vsr from pt448_ablation_full), sweeps
alpha in STEERING_ALPHAS and records ∆VSR, ∆Ctrl, ∆VQA at each alpha.
Negative alpha = suppress (should mirror ablation drop).
Positive alpha = amplify (target: VSR gain above pt-448 baseline).

Usage:
    python3 pt448_steering.py \
        --ablation-csv /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_ablation_full/ablation_summary.csv \
        --top-n 20 --gpus 0 1 2 3
"""
...
STEERING_ALPHAS = [-20.0, -10.0, -5.0, 5.0, 10.0, 20.0, 50.0]  # negative = suppress, positive = amplify
...
def _ablation_worker(gpu_id, feature_assignments, out_dir, vqa_max):
    ...
    def _do_steering_trace(input_ids, attn_mask, pixel_values, feature_vec, img_end, alpha):
        """Amplify feature direction at all 3 taps across 26 layers, text tokens only.
        True mirror of 3-point ablation: adds alpha * projection onto feature_vec,
        so the intervention scales with actual feature activation per sample.
        All ops stay proxy-based so NNsight intercepts them correctly.
        alpha=-1 ≈ ablation (removes projection); alpha=+1 doubles it.
        """
...
def main():
    ...
    parser.add_argument("--ablation-csv", type=str,
                        default=str(ANALYSIS_DIR / "pt448_ablation_full" / "ablation_summary.csv"),
                        help="pt-448 ablation_summary.csv (has layer, feature, relations, delta_vsr)")
    ...
    # Load top-N features by pt-448 delta_vsr (most negative = most impactful in backbone)
    rows = []
    with open(args.ablation_csv) as f:
        for row in csv.DictReader(f):
            row["layer"] = int(row["layer"])
            row["feature"] = int(row["feature"])
            row["delta_vsr"] = float(row["delta_vsr"])
            rows.append(row)
    rows.sort(key=lambda r: r["delta_vsr"])  # most negative first
    top_rows = rows[:args.top_n]
    features = [
        (r["layer"], r["feature"], r["relations"], float(r["odds_ratio"]))
        for r in top_rows if r.get("relations", "").strip()
    ]
    print(f"Loaded {len(features)} features (top-{args.top_n} by pt-448 delta_vsr)")
    print(f"\nFeatures to steer:")
    for l, f, rels, or_ in features:
        dv = next(r["delta_vsr"] for r in top_rows if r["layer"] == l and r["feature"] == f)
        print(f"  L{l}/F{f}: ∆VSR(pt-448)={dv:+.2f}%  OR={or_:.1f}  rels={rels[:60]}")

    out_dir = Path(args.out_dir) if args.out_dir else ANALYSIS_DIR / "pt448_steering"
    out_dir.mkdir(parents=True, exist_ok=True)
