#!/usr/bin/env python3
"""
Test L9/F7540 on all oracle-assigned relations.

L9/F7540 is assigned to: consists_of, in_the_middle_of, next_to, on, opposite_to, parallel_to
Subset sweep on "consists_of" showed: best_delta=0.00% (unsteerable at N=35).

This script tests L9/F7540 on each of its 5 remaining assigned relations to check:
1. Does it help any of them at oracle alpha=0.25?
2. Is there a better alpha?
3. Should these be moved to "skip" or assigned to a different feature?

Also tests L12/F2257 and L11/F12278 on these same relations for cross-feature comparison.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L9F7540_assigned_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_L9F7540_assigned_relations_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L9F7540_assigned_sweep")
...
# Relations currently assigned to L9/F7540 in the oracle (excluding consists_of already tested)
TARGET_RELATIONS = [
    "in the middle of",
    "next to",
    "on",
...
# Features to test on these relations
FEATURE_CONFIGS = {
    "L9_F7540":  {"layer": 9,  "feature": 7540,  "start": 9,  "oracle_alpha": 0.25,
...
def main():
    ...
    for rel in TARGET_RELATIONS:
        ...
        for feat_key, cfg in FEATURE_CONFIGS.items():
            ...
            rel_results["features"][feat_key] = {
                ...
                "oracle_delta": feat_results.get(str(oracle_alpha), {}).get("delta_acc", None),
                "alphas": feat_results,
            }
    ...
    # Summary table
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':20s}  {'N':5s}  {'Base':7s}  L9/F7540(oracle)  L12/F2257  L11/F12278", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        base = r.get("base_acc", 0)
        feats = r.get("features", {})
        d7540_oracle = feats.get("L9_F7540", {}).get("oracle_delta")
        d7540_best = feats.get("L9_F7540", {}).get("best_delta")
        d2257 = feats.get("L12_F2257", {}).get("best_delta")
        d12278 = feats.get("L11_F12278", {}).get("best_delta")
        d7540_str = f"{d7540_oracle:+.2f}%(ora)/{d7540_best:+.2f}%(best)" if d7540_oracle is not None else "—"
        print(f"{rel:20s}  {r.get('n',0):5d}  {base:7.2f}%  {d7540_str:25s}  {d2257 if d2257 else 0:+.2f}%  {d12278 if d12278 else 0:+.2f}%", flush=True)
