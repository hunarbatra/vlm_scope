#!/usr/bin/env python3
"""
Smart Oracle v10 — Extended per-relation alpha overrides, 10 total.

Changes from v9:
- L11/F12278 "under": α=0.75 → +1.87% (vs oracle α=0.45 → +1.19%, +0.68pp, N=589)
- All v9 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v10/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v10.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v10")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "under":       0.75,   # +1.87% vs oracle +1.19%, +0.68pp, N=589 (NEW in v10)
    # L15/F220 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v10_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v9 expected ~+4.55%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v10 — 10 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v10] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v9 expected ~+4.55%;  v8 ~+4.45%;  v7 ~+4.35%;  v6 ~+4.30%;  v5 ~+4.20%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v10", "alpha_mode": "per_relation_alpha_10overrides",
