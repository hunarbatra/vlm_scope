#!/usr/bin/env python3
"""
Subset sweep for extra CAA vectors (L13/F15219, L11/F9639) on their own-relation subsets.

L13/F15219: "behind" — coverage gap in canonical 8
L11/F9639: "in", "inside", "on" — large relation groups

These features were extracted but not in the canonical 8. This tests whether
their own-relation steering works where the canonical 8 fail.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_extra_features_subset_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_extra_features_subset_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_extra_features_subset_sweep")
...
FEATURE_CONFIGS = {
    "L13_F15219": {
        "layer": 13, "feature": 15219, "start": 0,
        "own_rels": ["behind"],
        "alphas": [0.1, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.75, 1.0, 1.25, 1.5, 2.0, 3.0],
    },
    "L11_F9639": {
        "layer": 11, "feature": 9639, "start": 0,
        "own_rels": ["in", "inside", "on"],
        "alphas": [0.1, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.75, 1.0, 1.25, 1.5, 2.0],
    },
}
...
def main():
    ...
    for rel in ["behind", "in", "inside", "on"]:
    ...
    for feat_key, cfg in FEATURE_CONFIGS.items():
        ...
        if not own_indices:
            print(f"[SKIP] {feat_key} no samples for {own_rels}", flush=True); continue
