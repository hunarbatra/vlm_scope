#!/usr/bin/env python3
"""
Step 10 (safety) — per-feature ablation on HallusionBench + VQA control.

For each feature F in features_to_ablate.csv:
  1. Regenerate HallusionBench responses with F ablated (3-point projection:
...
Outputs:
  analysis_hallucination/ablation_results/responses_L{L}_F{F}.jsonl
  analysis_hallucination/ablation_results/vqa_L{L}_F{F}.json
  analysis_hallucination/ablation_results/vqa_baseline_gpu{G}.json
...
JUDGE_FILE     = ANALYSIS_DIR / "judgments" / "mix448_hallusionbench_judgments.jsonl"
OUT_DIR        = ANALYSIS_DIR / "ablation_results"
OUT_DIR.mkdir(parents=True, exist_ok=True)

N_HB_EVAL = 100   # default; CLI override possible
...
def _load_judged_by_label(label_value):
    per_sc = defaultdict(list)
    with open(JUDGE_FILE) as f:
        for line in f:
            r = json.loads(line)
            if r.get("pred_label") == label_value:
                per_sc[r["subcategory"]].append((r["uid"], r["gt_answer"]))
    return per_sc


def load_eval_set():
    """INCORRECT-judged samples (target — test if ablation flips INCORRECT→CORRECT)."""
    per_sc = _load_judged_by_label("INCORRECT")
    total = sum(len(v) for v in per_sc.values())
    if N_HB_EVAL is None or N_HB_EVAL >= total:
        out = []
        for sc, items in per_sc.items():
            for uid, gt in items:
                out.append((uid, sc, gt))
        return out
    stratified = []
    for sc, items in per_sc.items():
        n = max(1, round(N_HB_EVAL * len(items) / total))
        stride = max(1, len(items) // n)
        picks = items[::stride][:n]
        for uid, gt in picks:
            stratified.append((uid, sc, gt))
    return stratified[:N_HB_EVAL]


def load_correct_control_set():
    """CORRECT-judged samples (specificity control — ablation should KEEP these correct)."""
    per_sc = _load_judged_by_label("CORRECT")
    out = []
    for sc, items in per_sc.items():
        for uid, gt in items:
            out.append((uid, sc, gt))
...
def _ablation_worker(gpu_id, feature_rows, n_hallusionbench=None, out_dir=None):
    global N_HB_EVAL, OUT_DIR
    if n_hallusionbench is not None:
        N_HB_EVAL = n_hallusionbench
    ...
    ds_hb = load_dataset("lmms-lab/HallusionBench", split="image")
    eval_tuples = load_eval_set()
    eval_set = {uid for uid, _, _ in eval_tuples}
    uid_to_gt = {uid: gt for uid, _, gt in eval_tuples}
    hb_dsidxs = [i for i in range(len(ds_hb)) if _uid(ds_hb[i]) in eval_set]
    print(f"[GPU{gpu_id}] HallusionBench INCORRECT eval set: {len(hb_dsidxs)} samples", flush=True)

    correct_tuples = load_correct_control_set()
    correct_set = {uid for uid, _, _ in correct_tuples}
    hb_correct_idxs = [i for i in range(len(ds_hb)) if _uid(ds_hb[i]) in correct_set]
    print(f"[GPU{gpu_id}] HallusionBench CORRECT control set: {len(hb_correct_idxs)} samples", flush=True)
    ...
    for feat_i, row in enumerate(feature_rows):
        ...
        subc = row.get("selected_for_subcategory", row.get("primary_subcategory", "?"))
        is_ctrl = int(row["is_control"])

        resp_path     = OUT_DIR / f"responses_L{layer_idx}_F{feature_idx}.jsonl"
        resp_ctrl_path = OUT_DIR / f"responses_correct_L{layer_idx}_F{feature_idx}.jsonl"
        vqa_path       = OUT_DIR / f"vqa_L{layer_idx}_F{feature_idx}.json"
        if resp_path.exists() and vqa_path.exists() and resp_ctrl_path.exists():
        ...
        print(f"[GPU{gpu_id}] [{feat_i+1}/{len(feature_rows)}] L{layer_idx}/F{feature_idx} "
              f"subc={subc} ctrl={is_ctrl}", flush=True)

        hooks = AblationHooks(model, feature_vec, img_end_ref)
        hooks.register()
        try:
            # --- HallusionBench ablated generations on INCORRECT eval set (target) ---
            if not resp_path.exists():
                with open(resp_path, "w") as outf:
                    for i, dsidx in enumerate(hb_dsidxs):
                        sample = ds_hb[dsidx]
                        uid = _uid(sample)
                        try:
                            ...
                            rec = {"uid": uid, "response": None,
                                   "status": f"error: {e}"}
                        outf.write(json.dumps(rec, ensure_ascii=False) + "\n")
                        outf.flush()

            # --- HallusionBench ablated generations on CORRECT control set (specificity) ---
            if not resp_ctrl_path.exists():
                with open(resp_ctrl_path, "w") as outf:
                    for i, dsidx in enumerate(hb_correct_idxs):
                        sample = ds_hb[dsidx]
                        uid = _uid(sample)
                        try:
                            prompt = f"answer en {sample['question']}"
            ...
            if not vqa_path.exists():
                ...
                vqa_res["selected_for_subcategory"] = subc
...
def main():
    ...
    parser.add_argument("--n-hallusionbench", type=int, default=N_HB_EVAL,
                        help="HallusionBench eval size; >=835 uses all baseline-UNSAFE")
    parser.add_argument("--out-dir", type=str, default=str(OUT_DIR))
    args = parser.parse_args()

    N_HB_EVAL = args.n_hallusionbench
    OUT_DIR = Path(args.out_dir); OUT_DIR.mkdir(parents=True, exist_ok=True)
    feats_path = Path(args.features)
    if not feats_path.exists():
        print(f"[FATAL] {feats_path} missing"); return
    df = pd.read_csv(feats_path)
    print(f"[MAIN] features={feats_path.name}  N_HB_EVAL={N_HB_EVAL}  out={OUT_DIR}")
    ...
    for gpu_id, shard in zip(args.gpus, shards):
        ...
        p = mp.Process(target=_ablation_worker,
                       args=(gpu_id, shard, N_HB_EVAL, str(OUT_DIR)))
