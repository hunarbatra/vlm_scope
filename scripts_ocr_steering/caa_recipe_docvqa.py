#!/usr/bin/env python3
"""
Recipe comparison — mix-src → pt-448, per-feature R(F)∩test subsets for DocVQA.

Mirrors caa_recipe_compare_mix_to_pt_devtest.py exactly, but for DocVQA:
  - MEANPOOL_DIR: analysis_docvqa/mix_hidden_cache/vi_{si:05d}.pt
  - SAE_ACTS_DIR: analysis_docvqa/sae_acts/acts_L{L}_F{F}.json
  - Labels: "correct" bool stored in each vi_NNNNN.pt
  - Correctness at eval: model response contains/is contained by gt_answers
  - Train split: splits.json["train"] (0..4278); Test split: splits.json["test"] (4279..5348)

Recipes evaluated:
  A. MIDDLE              α · unit(v_CAA[L13]) @ L13
  B. CAA_SAE_DOWN        α · unit(v_CAA[lF]) @ lF..25
  C. BACKBONE            α · unit(v_CAA[L]) @ each L in {15,17,19,21}
  D. BACKBONE+WDEC       (C) plus γ · W_dec[F] at lF only
  E. SPATIAL_LAYER       α · unit(v_CAA[lF]) @ lF only
  F. SPATIAL+WDEC        (E) plus γ · W_dec[F] at lF

Usage:
    CUDA_VISIBLE_DEVICES=X python3 caa_recipe_docvqa.py
"""
...
MEANPOOL_DIR = SAE_ROOT / "analysis_docvqa/mix_hidden_cache"
SAE_ACTS_DIR = SAE_ROOT / "analysis_docvqa/sae_acts"
SPLITS_JSON  = SAE_ROOT / "analysis_docvqa/splits.json"
OUT_DIR      = SAE_ROOT / "analysis_docvqa/caa_recipe_results"
...
MIDDLE_LAYER  = 13
CACHED_LAYERS = [16, 17, 18, 19, 21]   # DocVQA backbone (feature layers)
NUM_LAYERS    = 26
ALPHAS        = [0.5, 1.0, 2.0, 5.0]
GAMMAS        = [1.0, 3.0, 10.0]
MAX_NEW_TOKENS = 64

# DocVQA-specific features from MMDiff pipeline (top300_docvqa_features.csv)
SPATIAL_FEATURES = [
    {"layer": 16, "feature": 2825,  "key": "L16_F2825"},
    {"layer": 21, "feature": 12658, "key": "L21_F12658"},
    {"layer": 19, "feature": 6922,  "key": "L19_F6922"},
    {"layer": 18, "feature": 8613,  "key": "L18_F8613"},
    {"layer": 17, "feature": 3672,  "key": "L17_F3672"},
]
...
def main():
    ...
    print("=" * 80)
    print("RECIPE COMPARE — mix-src → pt-448, DocVQA R(F)∩test subsets")
    print("=" * 80, flush=True)
    ...
    for sf in SPATIAL_FEATURES:
        ...
        for si in rF[k]["indices"]:
            ...
            try:
                ...
                resp = tok.decode(out_ids[0, iids.shape[1]:], skip_special_tokens=True)
                bt += 1
                bc += int(_correct_docvqa(resp, gt))
            except Exception:
                continue
    ...
    # Summary
    print(f"\n{'='*120}\nSUMMARY — DocVQA R(F)∩test per-feature, best Δ per recipe\n{'='*120}", flush=True)
