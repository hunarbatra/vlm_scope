#!/usr/bin/env python3
"""
Per-example mix-448 SAE activation transfer to pt-448.

Most principled injection approach: instead of a fixed vector, for each example
we extract how strongly mix-448 fires feature F (the actual SAE activation),
then inject exactly that amount into pt-448.

    mix_act_F = mean(SAE.encode(h_mix[L])[text_tokens, F])   # scalar, per example
    pt-448 injection: layer_out += alpha * mix_act_F * W_dec[F]

Why this is better than fixed injection:
- Fixed injection (Exp 2): same magnitude for every example regardless of image content
- This: examples where mix-448 sees strong spatial signal get more injection;
  examples where mix-448 sees no spatial signal get near-zero injection
- This "routes" the mix-448 spatial perception into pt-448's processing

When alpha=1.0: inject exactly the mix-448 firing level into pt-448
When alpha=2.0: inject 2× the mix-448 level (boosted)

Also tests alpha<0: suppress the mix-448 signal in pt-448 (cross-model ablation)

Compared to SAE-calibrated injection (Exp 4) which used dataset means,
this uses per-example activations, giving much sharper signal.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_mix_activation_transfer/

Usage:
    CUDA_VISIBLE_DEVICES=4 python3 pt448_mix_activation_transfer.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_mix_activation_transfer")
...
# alpha=1.0 = inject exactly the mix-448 SAE activation magnitude
INJECTION_ALPHAS = [-1.0, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0]
...
def main():
    ...
    def extract_mix_activations(indices, layer_idx, feature_idx, sae):
        """For each example: get mean SAE feature F activation from mix-448."""
        acts = {}
        for vi in indices:
    ...
    def run_vsr_baseline(indices):
        ...
        for vi in indices:
            ...
            try:
                input_ids, attn_mask, pixel_values = process_vlm_inputs(
                    img, prompt, pt_proc, pt_model, device=device)
    ...
    def run_vsr_transfer(indices, fv, layer_idx, mix_acts, alpha):
        """Inject alpha * mix_act[vi] * fv into pt-448 for each example vi."""
        correct = total = 0; margins = []
        ...
        for vi in indices:
            ...
            label = int(ex.get("label", 0))
            prompt = _build_vsr_prompt(str(ex.get("caption", "")))
            mix_act = mix_acts.get(vi, 0.0)
            injection = alpha * mix_act
            try:
                input_ids, attn_mask, pixel_values = process_vlm_inputs(
                ...
                with pt_nns.trace(input_ids=input_ids, attention_mask=attn_mask,
                    ...
                    # Proxy trick: use mix_act as constant via zeroing and adding
                    scale_proxy = (layer_out @ fv_col) * 0.0 + injection
                    layer_out += scale_proxy * fv
    ...
    for layer_idx, feature_idx, relations in TOP10:
        ...
        result_path = OUT_DIR / f"transfer_{key}.json"
        ...
        if rel_key not in baseline_cache:
            print(f"[BASE] pt-448 [{rel_key}] N={len(indices)}...", flush=True)
            acc, margin, n = run_vsr_baseline(indices)
            baseline_cache[rel_key] = (acc, margin, n)
            print(f"[BASE] {acc:.2f}% margin={margin:.3f}", flush=True)
        ...
        # Extract per-example mix-448 SAE activations
        print(f"[EXTRACT] {key}: extracting mix-448 act_F for {len(indices)} examples...", flush=True)
        mix_acts = extract_mix_activations(indices, layer_idx, feature_idx, sae)
        del sae; torch.cuda.empty_cache()

        act_vals = list(mix_acts.values())
        act_mean = sum(act_vals) / max(len(act_vals), 1)
        act_max  = max(act_vals) if act_vals else 0
        print(f"[EXTRACT] {key}: mean_act={act_mean:.3f} max_act={act_max:.3f} "
              f"coverage={len(mix_acts)}/{len(indices)}", flush=True)
        ...
        result = {
            ...
            "mix_act_mean": act_mean, "mix_act_max": act_max,
            "method": "per_example_mix_act_transfer", "alphas": {}
        }

        for alpha in INJECTION_ALPHAS:
            print(f"[TRANSFER] {key} α={alpha:+g} (N={len(indices)})...", flush=True)
            acc, margin, n = run_vsr_transfer(indices, fv, layer_idx, mix_acts, alpha)
            da = acc - base_acc; dm = margin - base_margin
            result["alphas"][str(alpha)] = {
                "acc": acc, "delta_acc": da, "margin": margin,
                "delta_margin": dm, "n": n,
                "effective_injection_mean": alpha * act_mean
            }
            print(f"  α={alpha:+g} (inj≈{alpha*act_mean:.2f}): {acc:.2f}% "
                  f"(Δ={da:+.2f}%) margin={margin:.3f} (Δ={dm:+.3f})", flush=True)
    ...
    print(f"\n{'='*115}")
    print("pt-448 ← mix-448 Per-Example SAE Activation Transfer")
    print(f"{'='*115}")
    header = f"{'L/F':<12} {'Relations':<32} {'N':>5} {'Base':>7} {'MixAct':>7}"
    for a in INJECTION_ALPHAS: header += f"  {a:>+6}"
    print(header); print("-" * 115)
    ...
    for r in all_results:
        ...
        row = (f"{key:<12} {rels:<32} {r['n_samples']:>5} "
               f"{r['baseline_vsr_acc']:>6.1f}% {r.get('mix_act_mean',0):>7.3f}")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for r in all_results:
            row = {"layer":r["layer"],"feature":r["feature"],
                   ...
                   "mix_act_mean":f"{r.get('mix_act_mean',0):.4f}",
                   "mix_act_max":f"{r.get('mix_act_max',0):.4f}"}
            for a in INJECTION_ALPHAS:
