#!/usr/bin/env python3
"""
Cross-feature sweep for L9_F7540-assigned relations: on, next_to, parallel_to, in_middle_of.
These are currently assigned to L9_F7540 but have never been compared against other features.
Sweeps: L9_F7540(oracle), L12_F2257, L15_F220, L11_F12278, L6_F7539.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_l9f7540_relations_sweep/
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_l9f7540_relations_sweep")
...
TARGET_RELATIONS = ["on", "next to", "parallel to", "in the middle of"]
...
def main():
    ...
    for rel in TARGET_RELATIONS:
        ...
        best_feat = max(feat_results, key=lambda k: feat_results[k]["best_delta"], default="?")
        best_d = feat_results[best_feat]["best_delta"] if best_feat in feat_results else 0
        summary_rows.append((rel, n, base_acc, best_feat, feat_results[best_feat]["best_alpha"] if best_feat in feat_results else "?", best_d))

    print("\n" + "="*60, flush=True)
    print(f"{'Relation':<25} {'N':>5} {'Base':>7} {'BestFeat':<15} {'BestAlpha':>9} {'BestDelta':>9}", flush=True)
    for rel, n, base, feat, alpha, delta in summary_rows:
        print(f"{rel:<25} {n:>5} {base:>6.2f}% {feat:<15} {alpha:>9} {delta:>+9.2f}%", flush=True)
