#!/usr/bin/env python3
"""
Smart Oracle v16 — 22 per-relation alpha overrides (+1 new).

Changes from v15:
- "inside" feature reassigned L15_F220 → L12_F2257
  [L12 oracle α=0.75 → +2.92% vs L15 best +1.67%, +1.25pp, N=240, reassignment_sweep confirmed]
- "outside" alpha override: L12_F2257 α=1.0 → +9.38% vs oracle +3.12%, +6.26pp, N=32
  [N=32 is small but +6.26pp gap is large; caution flag noted]
- All v15 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v16/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v16.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v16")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "outside":        1.0,  # +9.38% vs oracle +3.12%, +6.26pp, N=32 (NEW v16, N small)
    # L9/F387 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v16_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v15 expected ~+5.10%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v16 — 22 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v16] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v15 expected ~+5.10%;  v14 ~+5.00%;  v13 ~+4.90%;  v12 ~+4.82%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v16", "alpha_mode": "per_relation_alpha_22overrides",
