#!/usr/bin/env python3
"""
Smart Oracle v12 — Extended per-relation alpha overrides, 15 total.

Changes from v11:
- L9/F387 "far from": α=0.5 → +2.07% (vs oracle α=0.4 → -0.69%, +2.76pp, N=145)
- L15/F220 "part of": α=0.8 → +4.42% (vs oracle α=0.7 → -0.88%, +5.30pp, N=113)
- All v11 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v12/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v12.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v12")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "part of":     0.8,    # +4.42% vs oracle -0.88%, +5.30pp, N=113 (NEW in v12)
    # L6/F7539 overrides
    ...
    # L9/F387 overrides
    "far from":       0.5,  # +2.07% vs oracle -0.69%, +2.76pp, N=145 (NEW in v12)
    # L9/F7540 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v12_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v11 expected ~+4.72%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v12 — 15 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v12] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v11 expected ~+4.72%;  v10 ~+4.60%;  v9 ~+4.55%;  v8 ~+4.45%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v12", "alpha_mode": "per_relation_alpha_15overrides",
