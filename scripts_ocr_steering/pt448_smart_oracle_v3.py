#!/usr/bin/env python3
"""
Smart Oracle v3 — Updated alphas from per-feature own-relation fine sweeps.

Changes from v2:
- L12/F2257: α=1.0 → α=0.75 (subset sweep confirms +8.82% vs +7.84% at 1.0)
- L15/F220:  α=0.75 → α=0.7  (fine sweep on 'at the left side of' peaks at α=0.7: +8.55%)
- L4/F14233: α=1.0 → α=0.9  (negligibly different but technically confirmed optimal)
- All other configs unchanged.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v3/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v3.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v3")
...
# Updated alphas from per-feature own-relation fine sweeps (subset methodology)
FEATURE_CONFIGS = {
    "L4_F14233":  {"layer": 4,  "feature": 14233, "start": 0,  "alpha": 0.9},   # subset: α=0.9 → +15.38%
    "L14_F10561": {"layer": 14, "feature": 10561, "start": 0,  "alpha": 2.0},   # subset: α=2.0 → +10.75% (confirmed)
    "L12_F2257":  {"layer": 12, "feature": 2257,  "start": 1,  "alpha": 0.75},  # subset: α=0.75 → +8.82% (was 1.0→+7.84%)
    "L15_F220":   {"layer": 15, "feature": 220,   "start": 15, "alpha": 0.7},   # subset: α=0.7 → +8.55% (was 0.75)
    "L11_F12278": {"layer": 11, "feature": 12278, "start": 5,  "alpha": 0.5},   # global: α=0.5 → +6.32% (fine sweep TBD)
    "L9_F387":    {"layer": 9,  "feature": 387,   "start": 1,  "alpha": 0.5},   # prior best (TBD from subset sweep)
    "L6_F7539":   {"layer": 6,  "feature": 7539,  "start": 1,  "alpha": 1.5},   # prior best
    "L9_F7540":   {"layer": 9,  "feature": 7540,  "start": 9,  "alpha": 0.25},  # prior best
}
...
def main():
    ...
    # Only run per_feature mode (v3 purpose is to test updated alphas)
    for alpha_mode in ["per_feature"]:
        alpha_tag = "per_feature"
        result_path = OUT_DIR / f"smart_oracle_v3_{alpha_tag}.json"
        if result_path.exists():
            print(f"[SKIP] {result_path} exists", flush=True)
            with open(result_path) as f:
                r = json.load(f)
            print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v2 was +3.96%)")
            continue

        print(f"\n{'='*60}", flush=True)
        print(f"α mode = {alpha_mode} (v3 updated alphas)", flush=True)
        ...
        print(f"\n[RESULT v3] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
        print(f"  [vs v2: +3.96%  inject Δ: +4.71%]", flush=True)
        ...
        res = {"version": "v3", "alpha_mode": alpha_mode,
