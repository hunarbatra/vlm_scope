#!/usr/bin/env python3
"""
Fixed-vector feature injection for pt-448 using mix-448 spatial feature directions.

For each top-10 feature, injects the mix-448 W_dec direction into pt-448 at the
native SAE layer only (layer_out / residual stream), text tokens only:

    layer_out[img_end:] += alpha * fv

NNsight proxy trick ensures the addition is captured correctly:
    ones = (layer_out @ fv_col) * 0.0 + 1.0   # (T, 1) proxy = 1.0
    layer_out += alpha * ones * fv_flat         # (T, d) proxy injection ✓

This differs from projection-based steering (which amplifies existing firing,
ineffective when the feature barely fires in pt-448). Fixed injection adds
the direction regardless of current activation.

alpha=-1  ≈ weak removal sanity check
alpha>0   = implant feature direction into pretrained backbone

Usage:
    CUDA_VISIBLE_DEVICES=0 python3 pt448_feature_injection.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_feature_injection")
...
# Positive = implant spatial signal; -1 = sanity (mild removal)
...
def main():
    ...
    def run_vsr(indices, feature_vec=None, layer_idx=None, alpha=0.0):
        """Run VSR eval; if feature_vec given, inject at layer_idx residual stream."""
        correct = total = 0
        for vi in indices:
            ex = vsr_all[vi]
            img = _load_image(ex)
            ...
            try:
                ...
                if feature_vec is None:
                    ...
                    # Proxy trick: derive (T,1) ones-proxy via matmul with fv, then zero and add 1.
                    # This ensures NNsight intercepts the injection (proxy += proxy).
                    fv_flat = feature_vec          # (d,)
                    fv_col  = fv_flat.unsqueeze(1) # (d, 1) for matmul
                    with nns_model.trace(input_ids=input_ids, attention_mask=attn_mask,
                        ...
                        # (T, d) @ (d, 1) → (T, 1) proxy; zero it; add 1 → ones proxy
                        ones = (layer_out @ fv_col) * 0.0 + 1.0
                        # Add alpha * fv to every text token position
                        layer_out += alpha * ones * fv_flat
    ...
    for layer_idx, feature_idx, relations in TOP10:
        ...
        result_path = OUT_DIR / f"injection_{key}.json"
        ...
        # Load SAE feature direction from mix-448 SAE checkpoints
        ...
        for alpha in INJECTION_ALPHAS:
            ...
            acc, n = run_vsr(indices, fv, layer_idx, alpha)
            delta = acc - base_acc
    ...
    # Print summary table
    print(f"\n{'='*110}")
    print(f"pt-448 Fixed-Vector Injection — Top-10 Spatial Features (mix-448 W_dec directions)")
    print(f"{'='*110}")
    ...
    import csv
    csv_path = OUT_DIR / "injection_summary.csv"
