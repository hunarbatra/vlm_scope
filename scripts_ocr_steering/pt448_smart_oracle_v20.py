#!/usr/bin/env python3
"""
Smart Oracle v20 — 25 per-relation alpha overrides (+1 new), 1 feature reassignment.

Changes from v19:
- "attached to" feature reassigned L9_F387 → L15_F220 + alpha override α=0.6
  [L15 best α=0.6 → +8.93% vs L9 oracle +7.14%, +1.79pp, N=56, unswept_relations_sweep confirmed]
- All v19 feature assignments and overrides retained

Notes on completed sweeps (no changes needed):
- "over" (N=84): L15_F220 oracle α=0.7 → +8.33% already best
- "contains" (N=343): L15_F220 oracle α=0.7 → +6.12% already best
- "at_left_side_of" (N=421): L15 oracle +8.55% confirmed best (L9 ties but no improvement)
- "on" (N=585): L9_F7540 best α=0.3 → +1.37% still best

Total: 25 overrides (+1 new), 1 feature reassignment (attached_to L9_F387→L15_F220)

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v20/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v20.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v20")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "touching":    0.5,    # +5.54% vs oracle +5.01%, +0.53pp, N=1281
    "under":       0.75,   # +1.87% vs oracle +1.19%, +0.68pp, N=589
    # L15/F220 overrides
    ...
    # L15/F220 overrides (continued)
    "attached to":    0.6,  # +8.93% vs L9 oracle +7.14%, +1.79pp, N=56 (NEW v20, feat reassigned)
    # L6/F7539 overrides
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v20_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v19 expected ~+5.23%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v20 — 25 per-relation alpha overrides (+1 new), 1 reassignment", flush=True)
    ...
    print(f"\n[RESULT v20] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v19 expected ~+5.23%;  v18 ~+5.20%;  v17 ~+5.20%;  v16 ~+5.15%;  v15 ~+5.10%]", flush=True)
    ...
    res = {"version": "v20", "alpha_mode": "per_relation_alpha_25overrides_v20",
