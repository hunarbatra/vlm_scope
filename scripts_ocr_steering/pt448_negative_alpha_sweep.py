#!/usr/bin/env python3
"""
Negative alpha sweep for features with negative SAE contrast.

Four features fire MORE on false spatial examples than true ones in mix-448:
  L9/F387   "at the right side of" contrast=-0.058 (fires more on false)
  L14/F10561 "close to"            contrast=-0.195 (fires more on false)
  L6/F7539  "left of; right of"    contrast=-0.003 (fires slightly more on false)
  L11/F9639 "in; inside; on"       contrast=-0.240 (fires more on false)

For these features, +W_dec direction corresponds to spatial ABSENCE/mismatch,
not presence. Therefore NEGATIVE injection (-alpha * W_dec) should be the
correct direction to activate spatial PRESENCE in pt-448.

Tests NEGATIVE alpha range with "all_ml" strategy (26 layers, 0.7 decay).
Also tests "single" at native SAE layer for comparison.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_neg_alpha/

Usage:
    CUDA_VISIBLE_DEVICES=4 python3 pt448_negative_alpha_sweep.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_neg_alpha")
...
NEG_CONTRAST_FEATURES = [
    (9,  387,   ["at the right side of"],  -0.058,  "all_ml"),
    (14, 10561, ["close to"],              -0.195,  "all_ml"),
    (6,  7539,  ["left of","right of"],    -0.003,  "all_ml"),
    (11, 9639,  ["in","inside","on"],      -0.240,  "all_ml"),
]

# Test both negative AND positive to see the full picture
ALPHA_RANGE = [-100.0, -50.0, -30.0, -20.0, -10.0, -5.0, -2.0, -1.0,
                1.0,    2.0,   5.0,  10.0,  20.0,  30.0, 50.0]
...
def main():
    ...
    for layer_idx, feature_idx, relations, contrast, strategy in NEG_CONTRAST_FEATURES:
        key = f"L{layer_idx}_F{feature_idx}"
        result_path = OUT_DIR / f"negalpha_{key}.json"
        ...
        # all_ml layer weights
        lw = {l: DECAY ** abs(l - layer_idx) for l in range(N_LAYERS)}
        fv_col = fv.unsqueeze(1)

        result = {
            "layer": layer_idx, "feature": feature_idx, "relations": relations,
            "contrast": contrast, "strategy": strategy,
            "n_samples": len(indices), "baseline_vsr_acc": base_acc, "baseline_margin": base_mg,
            "alphas": {}
        }

        print(f"\n[SWEEP] {key} contrast={contrast:+.3f} (neg→inject -W_dec should help)", flush=True)
    ...
    print(f"\n{'='*100}")
    print("Negative Contrast Features — Signed Alpha Sweep")
    print(f"{'='*100}")
    for r in all_results:
        key = f"L{r['layer']}/F{r['feature']}"
        best_neg_a, best_neg_v = max(((a,v) for a,v in r["alphas"].items() if float(a)<0),
                                      key=lambda x: x[1]["delta_acc"], default=("NA",{"delta_acc":0}))
        best_pos_a, best_pos_v = max(((a,v) for a,v in r["alphas"].items() if float(a)>0),
                                      key=lambda x: x[1]["delta_acc"], default=("NA",{"delta_acc":0}))
        print(f"{key} {r['relations']} contrast={r['contrast']:+.3f}")
        print(f"  Best negative α: Δ={best_neg_v['delta_acc']:+.2f}% @ α={best_neg_a}")
        print(f"  Best positive α: Δ={best_pos_v['delta_acc']:+.2f}% @ α={best_pos_a}")
