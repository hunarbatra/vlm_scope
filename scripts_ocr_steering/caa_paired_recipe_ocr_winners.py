#!/usr/bin/env python3
"""
Recipe sweep using WINNER features (fire under ocr prompt).

Reads acts from sae_acts_ocrprompt/ (ocr-prompt SAE activations) for R(F).
Cache from paired_cache_ocrprompt/ (ocr-prompt paired contrast).

Recipes:
  A_MIDDLE   alpha · unit(v[L13]) at L13                                     (baseline)
  D_BB+WDEC  alpha · unit(v[L]) at {17,19,20,21} + gamma · W_dec[F] at lF   (mmdiff)

Eval: lenient substring metric (OCR-Bench official).

Usage:
  CUDA_VISIBLE_DEVICES=X python3 -B scripts/caa_paired_recipe_ocr_winners.py
"""
...
# Winners: features that fire under ocr prompt AND have ablation impact
SPATIAL_FEATURES = [
    {"layer": 21, "feature": 13072, "key": "L21_F13072"},  # 78% firing, -8.3% abl
    {"layer": 19, "feature": 9893,  "key": "L19_F9893"},   # 38% firing, -3.1% abl
    {"layer": 21, "feature": 677,   "key": "L21_F677"},    # 22% firing, -1.7% abl
    {"layer": 19, "feature": 8866,  "key": "L19_F8866"},   # 58% firing, -0.4% abl
]
...
def main():
    ...
    print("=" * 80)
    print("WINNER FEATURES — ocr prompt — pt-448 OCR-Bench R(F)∩all-1000")
    print("=" * 80, flush=True)
    ...
    for sf in SPATIAL_FEATURES:
        ...
        if not ap.exists():
            print(f"  [SKIP] {k}: acts file not yet ready ({ap})", flush=True)
            continue
    ...
    if not rF:
        print("[ERROR] No winner acts files yet. Wait for extraction.")
        return

    needed_layers = sorted(set(BACKBONE_LAYERS) | {MIDDLE_LAYER}
                           | {sf["layer"] for sf in SPATIAL_FEATURES if sf["key"] in rF})
    ...
    # summary
    print(f"\n{'='*90}\nSUMMARY — winners (ocr-firing features), per-feature CAA\n{'='*90}", flush=True)
