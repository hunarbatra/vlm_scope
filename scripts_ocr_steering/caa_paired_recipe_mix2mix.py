#!/usr/bin/env python3
"""mix→mix steering: use mix-448 for both source and target.

Expectation: mix already does OCR (~64% lenient baseline); steering should be
able to lift it further since the CAA direction is in-distribution.
"""
...
OUT_DIR      = SAE_ROOT / "analysis_ocr/caa_paired_mix2mix"
...
def main():
    ...
    print(f"=== mix→mix {key} ===", flush=True)
    ...
    if bk not in all_results:
        ...
        print(f"[{key}] mix-mix rF base: {all_results[bk]['acc']:.2f}% (n={bt})", flush=True)
        with open(results_path, "w") as f: json.dump(all_results, f, indent=2)

    base_rF = all_results[bk]["acc"]
    print(f"\n--- mix→mix sweep: {key} n={len(rF)} base={base_rF:.2f}% ---", flush=True)
    ...
    if MIDDLE_LAYER in caa_unit:
        for a in ALPHAS:
            ...
            all_results = run_eval(f"{key}/A_MIDDLE α={a}", [(MIDDLE_LAYER, sv)],
                rF, ds, mdl, proc, tok, device, base_rF, rk, all_results, results_path)
    ...
    for a in ALPHAS:
        for g in GAMMAS:
            ...
            all_results = run_eval(f"{key}/D_BB+WDEC α={a} γ={g}", inject,
                rF, ds, mdl, proc, tok, device, base_rF, rk, all_results, results_path)
