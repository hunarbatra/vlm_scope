#!/usr/bin/env python3
"""
Smart Oracle v14 — Extended per-relation alpha overrides, 21 total (+3 new).

Changes from v13:
- BEST_FEATURE_MAP: "at the side of" → L12_F2257 (+3.45% vs L9 +0.00%, +3.45pp, N=58)
- BEST_FEATURE_MAP: "away from" → L9_F7540 (+3.87% vs L15 +1.29%, +2.58pp, N=155)
  - away_from gets new alpha override: α=1.0 (vs L9_F7540 oracle α=0.25 → +0.65%)
- L14/F10561 "connected to": α=1.5 → +13.51% (vs oracle α=2.0 → +10.81%, +2.70pp, N=37)
- L15/F220 "away from" override: α=0.6 (for L15 if kept; obsolete with feature reassignment)
- All v13 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v14/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v14.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v14")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "connected to":   1.5,  # +13.51% vs oracle +10.81%, +2.70pp, N=37 (NEW v14)
    # L9/F7540 overrides
    ...
    "away from":        1.0, # +3.87% vs L9F7540 oracle +0.65%, +3.22pp, N=155 (NEW v14, feat reassigned)
}
...
BEST_FEATURE_MAP = {
    ...
    "at the side of":      "L12_F2257",   # Changed from L9_F387: +3.45% vs +0.00%, N=58
    "attached to":         "L9_F387",
    "away from":           "L9_F7540",    # Changed from L15_F220: +3.87% vs +1.29%, +2.58pp, N=155
    "behind":              "L4_F14233",
    "below":               "L6_F7539",
    "beneath":             "L12_F2257",
    "beside":              "L15_F220",
    ...
    "part of":             "L15_F220",
    "right of":            "L15_F220",    # Changed from L6_F7539: +1.77% vs +0.88%, N=113
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v14_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v13 expected ~+4.90%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v14 — 20 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v14] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v13 expected ~+4.90%;  v12 ~+4.82%;  v11 ~+4.72%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v14", "alpha_mode": "per_relation_alpha_20overrides",
