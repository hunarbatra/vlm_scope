#!/usr/bin/env python3
"""
Smart Oracle v13 — Extended per-relation alpha overrides, 18 total (+3 new).

Changes from v12:
- L9/F387 "adjacent to": α=0.5 → +5.19% (vs oracle α=0.4 → +2.60%, +2.59pp, N=77)
- L14/F10561 "by": α=0.5 → +7.69% (vs oracle α=2.0 → +3.85%, +3.84pp, N=52)
- L6/F7539 "alongside": α=1.25 → +14.55% (vs oracle α=1.5 → +12.73%, +1.82pp, N=55, confirmed 2x)
- BEST_FEATURE_MAP change: "right of" → L15_F220 (+1.77% vs L6 +0.88%, +0.89pp, N=113)
- All v12 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v13/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v13.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v13")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "alongside":      1.25, # +14.55% vs oracle +12.73%, +1.82pp, N=55 (NEW v13, confirmed 2x)
    # L12/F2257 overrides
    ...
    "adjacent to":    0.5,  # +5.19% vs oracle +2.60%, +2.59pp, N=77 (NEW v13)
    # L14/F10561 overrides
    "by":             0.5,  # +7.69% vs oracle +3.85%, +3.84pp, N=52 (NEW v13)
    # L9/F7540 overrides
...
BEST_FEATURE_MAP = {
    ...
    "part of":             "L15_F220",
    "right of":            "L15_F220",   # Changed from L6_F7539: +1.77% vs +0.88%, +0.89pp, N=113
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v13_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v12 expected ~+4.77%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v13 — 18 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v13] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v12 expected ~+4.77%;  v11 ~+4.72%;  v10 ~+4.60%;  v9 ~+4.55%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v13", "alpha_mode": "per_relation_alpha_18overrides",
