#!/usr/bin/env python3
"""
Test whether currently-SKIP relations benefit from CAA injection.
Tests: in front of (N=737), far away from (N=232), at the edge of (N=211), in (N=368)
against best features at a range of alphas.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_skip_relations_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_skip_relations_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_skip_relations_sweep")
...
TARGET_RELATIONS = [
    "in front of",    # N=737, spatial — might respond to L15 or L12
    "in",             # N=368, containment — might respond to L15/L12
    "far away from",  # N=232, distance — might respond to L9/F387
    "at the edge of", # N=211, position — might respond to L15/L12
]

FEATURE_CONFIGS = {
    "L15_F220":   {"layer": 15, "feature": 220,  "start": 15, "oracle_alpha": 0.7,
                   "alphas": [0.3, 0.5, 0.7, 0.9, 1.25]},
...
def main():
    ...
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':25s}  {'N':5s}  {'Base':7s}  {'Best feat/alpha':20s}  {'Best Δ':8s}", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        base = r.get("base_acc", 0)
        feats = r.get("features", {})
        best_feat = max(feats.items(), key=lambda x: x[1].get("best_delta", -999)) if feats else ("?", {})
        bd = best_feat[1].get("best_delta", 0) or 0
        ba_str = best_feat[1].get("best_alpha")
        inject = "INJECT" if bd > 1.0 else "SKIP"
        print(f"{rel:25s}  {r.get('n',0):5d}  {base:7.2f}%  {best_feat[0]:15s} α={ba_str}  {bd:+8.2f}%  [{inject}]", flush=True)
