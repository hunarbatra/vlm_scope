#!/usr/bin/env python3
"""
Sweep for relations with potential feature reassignments or alpha improvements:
  - inside: L15/F220 assigned, but L12/F2257 gives +2.92% vs L15 +1.67% (N=240)
  - toward: L12/F2257 assigned, but L15/F220 α=0.9 → +19.44% vs oracle +16.67% (N=36)
  - within: L15/F220 assigned, α=0.8 → +6.06% vs oracle α=0.7 → +3.03% (N=33)
  - outside: L12/F2257 assigned, α=1.0 → +9.38% vs oracle +3.12% (N=32)
  - left_of: L6/F7539 assigned, oracle +4.29% optimal (N=?)

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_reassignment_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_reassignment_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_reassignment_sweep")
...
TARGET_RELATIONS = [
    ...
    "outside",
    "left of",
    "left of",  # deduplicated below
]
# deduplicate
TARGET_RELATIONS = list(dict.fromkeys(TARGET_RELATIONS))

FEATURE_CONFIGS = {
    "L15_F220":  {"layer": 15, "feature": 220,  "start": 15, "oracle_alpha": 0.7,
                  "alphas": [0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.25, 1.5]},
...
def main():
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"{'Relation':20s}  {'N':5s}  {'Base':7s}  {'L15(oracle)':11s}  {'L12(oracle)':11s}", flush=True)
    print("-" * 70, flush=True)
    for rel, r in summary.items():
        ...
        d15 = f15.get("oracle_delta", 0) or 0
        d12 = f12.get("oracle_delta", 0) or 0
        print(f"{rel:20s}  {r.get('n',0):5d}  {base:7.2f}%  {d15:+11.2f}%  {d12:+11.2f}%", flush=True)
    print("\n[DONE]", flush=True)
