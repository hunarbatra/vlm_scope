#!/usr/bin/env python3
"""Wide-sweep: large + negative alphas/gammas. Tests if direction is flipped or
if much stronger steering helps.
"""
...
OUT_DIR      = SAE_ROOT / "analysis_ocr/caa_paired_wide_sweep"
...
# Wider sweep: include negative alphas and bigger positive ones
ALPHAS        = [-20.0, -10.0, -5.0, -2.0, 2.0, 5.0, 10.0, 20.0, 50.0]
GAMMAS        = [-10.0, -3.0, 3.0, 10.0, 30.0]
MAX_NEW_TOKENS = 64
...
def main():
    ...
    print(f"=== WIDE SWEEP {key} ===", flush=True)
    ...
    if bk not in all_results:
        ...
        print(f"[{key}] base: {all_results[bk]['acc']:.2f}% (n={bt})", flush=True)
        with open(results_path, "w") as f: json.dump(all_results, f, indent=2)
