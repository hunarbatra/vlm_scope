#!/usr/bin/env python3
"""MCQ + ocrprompt-cache D recipe for ablation-top-5 features, using answer-en R(F)."""
...
SAE_ACTS_DIR = SAE_ROOT / "analysis_ocr/sae_acts"  # answer-en R(F) — same as ablation
OUT_DIR      = SAE_ROOT / "analysis_ocr/mcq_top5_ablation_features"
...
def main():
    ...
    print(f"=== MCQ-D ablation-feature {key} ===", flush=True)
    ...
    print(f"R({key}) (answer-en) over all 1000: n={len(rF)}", flush=True)
