#!/usr/bin/env python3
"""
True CAA mix-src → steer mix-448 (cache-based).
...
This is the cache-based analogue of v4 (mix→mix live extraction), allowing
direct comparison with pt-src (pt→pt cache-based) using the same extraction
protocol. v4 used live extraction at last-token position; this uses the
cached mean-over-text-tokens. Comparison reveals extraction-protocol effect.

Usage:
    CUDA_VISIBLE_DEVICES=4 python3 -B pt448_true_caa_v4_mix_to_mix.py
"""
...
OUT_DIR        = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_true_caa_v4_mix_to_mix")
...
def main():
    ...
    print("=" * 70)
    print("True CAA mix-src → steer mix-448 (cache-based, mean-over-text-tokens)")
    ...
    print(f"\n{'='*70}")
    print("True CAA mix-src → mix-448 (cache)")
    print(f"{'='*70}")
