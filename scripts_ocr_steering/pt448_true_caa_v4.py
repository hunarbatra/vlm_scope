#!/usr/bin/env python3
"""
True CAA v4 — last-token extraction, mix-448 self-steering only.

Changes from v3:
  - Vector computed from LAST TEXT TOKEN hidden state (position -1 of text
    tokens, i.e. the "Answer:" token) rather than mean-over-text-tokens.
    This matches Rimsky et al. 2023 / Meg Tong's implementation exactly.
  - Source and target are both mix-448 (no cross-model transfer).
  - Live forward passes for vector extraction (cache is mean-aggregated,
    can't recover last-token from it).

Two conditions per feature F at layer lF:
  MIDDLE  v[13] from all 10,972 VSR samples. Last token.
  FEATURE v[lF] from R(F) ∩ {SAE feature F fires}. Last token.

Steers mix-448 on each R(F) subset.

Usage:
    CUDA_VISIBLE_DEVICES=0 python3 -B pt448_true_caa_v4.py
"""
...
OUT_DIR      = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_true_caa_v4")
...
def extract_last_token_hidden(model, processor, vsr_all, vis, layers, device):
    """
    Run mix-448 forward on each sample in vis, capture last text token
    hidden state at each requested layer. Returns dict {vi: {layer: tensor}}.

    'Last text token' = position index -1 of the text tokens (i.e. after
    img_end), which is the final "Answer:" colon token. This matches
    Rimsky et al. 2023 token-[-2] convention (last prompt token before
    the answer letter) as closely as possible for this prompt format.
    """
    ...
    for idx, vi in enumerate(vis):
        ...
        # Hook to capture last text token at each layer
        captures = {}
        hooks = []
        ...
        def make_hook(l):
            def hook_fn(module, inp, out):
                ...
                captures[l] = h[0, -1, :].detach().float().cpu()  # last token
            return hook_fn
...
def compute_vectors(model, processor, vsr_all, device):
    ...
    for layer, vis in sorted(layer_unique_vis.items()):
        ...
        layer_hidden[layer] = extract_last_token_hidden(model, processor, vsr_all, vis, [layer], device)
...
def main():
    ...
    print("=" * 70)
    print("True CAA v4 — last-token extraction — mix-448 self-steering")
    print(f"MIDDLE: L{MIDDLE_LAYER}, all 10,972 VSR samples")
    print(f"FEATURE: lF, R(F)∩fire_F — last text token hidden state")
    print("=" * 70, flush=True)
    ...
    # Summary
    print(f"\n{'='*70}")
    print("True CAA v4 — last-token — mix-448 self-steering")
    print(f"{'='*70}")
