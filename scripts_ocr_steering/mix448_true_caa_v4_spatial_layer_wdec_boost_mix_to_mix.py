#!/usr/bin/env python3
"""
True CAA SPATIAL_LAYER + W_dec[F] BOOSTED — mix→mix (self-steering).

Condition per feature F at layer lF:
  SPATIAL_LAYER_WDEC_BOOST
      unit( unit(v_spatial[lF]) + BETA · W_dec[F] ) injected at lF.
      BETA=3.0 gives W_dec[F] ~3x weight vs the data-driven contrastive
      direction. Compared to the plain WDEC variant (50/50 blend of two unit
      vectors), this tilts the combined direction toward the SAE's concept
      vector for feature F, to test whether stronger feature-direction
      signal helps.

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B mix448_true_caa_v4_spatial_layer_wdec_boost_mix_to_mix.py
"""
...
OUT_DIR         = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/mix448_true_caa_v4_spatial_layer_wdec_boost_mix_to_mix")
...
BETA   = 3.0  # W_dec[F] weight relative to v_spatial_unit
...
def compute_vectors(vsr_all):
    ...
    for sf in SPATIAL_FEATURES:
        ...
        combined = v_sl_unit + BETA * w_dec
        combined = combined / combined.norm().clamp(min=1e-8)
        cos_sim = (v_sl_unit * w_dec).sum().item()
        print(f"  [{key}] BOOST β={BETA}: cos(v_spatial, W_dec)={cos_sim:.4f}", flush=True)
        v_boost[key] = {"layer": lyr, "vec": combined}

    return v_spatial, v_boost
...
def main():
    ...
    print("=" * 70)
    print("True CAA SPATIAL_LAYER + W_dec[F] BOOST β=3 — mix→mix (self-steering)")
    print("=" * 70, flush=True)
    ...
    v_spatial, v_boost = compute_vectors(vsr_all)
    gc.collect()
    ...
    for sf in SPATIAL_FEATURES:
        ...
        if key in v_boost:
            lyr = v_boost[key]["layer"]
            print(f"  [{key}] SPATIAL_LAYER_WDEC_BOOST steer (L{lyr}→L{lyr})...", flush=True)
            all_results = run_steer_sweep(
                f"{key}/BOOST", v_boost[key]["vec"], lyr,
                rel_vis, rel_labels, base_acc,
                f"{key}_spatial_layer_wdec_boost", all_results, results_path,
                model, processor, yes_ids, no_ids, device, vsr_all,
            )
    ...
    print(f"\n{'='*70}")
    print(f"True CAA SPATIAL_LAYER_WDEC_BOOST β={BETA} — mix→mix")
    print(f"{'='*70}")
    print(f"  {'Feature':<16} {'N':>5} {'Base':>7}  {'BOOST Δ':>10}")
    print("  " + "-"*42)
    for sf in SPATIAL_FEATURES:
        key = sf["key"]
        ...
        r = all_results.get(f"{key}_spatial_layer_wdec_boost", {})
