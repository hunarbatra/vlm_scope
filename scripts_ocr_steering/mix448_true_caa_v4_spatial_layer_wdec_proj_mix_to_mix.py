#!/usr/bin/env python3
"""
True CAA SPATIAL_LAYER + W_dec[F] PROJECT+AMPLIFY — mix→mix (self-steering).

Condition per feature F at layer lF:
  SPATIAL_LAYER_WDEC_PROJ
      proj_F  = (v_spatial · W_dec[F]) · W_dec[F]           # component along F
      combined = v_spatial + (BETA - 1) · proj_F            # amplify only the F component
      inject unit(combined) at lF, scaled by alpha.
      BETA=3.0 → the W_dec[F] component of v_spatial is multiplied by 3x;
      BETA=1.0 would be identity. This keeps the steering vector on the
      manifold v_spatial already lives on — we never introduce any direction
      the data contrast didn't already identify. If cos(v_spatial, W_dec[F])
      ≈ 0, the condition degenerates to plain SPATIAL_LAYER.

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B mix448_true_caa_v4_spatial_layer_wdec_proj_mix_to_mix.py
"""
...
OUT_DIR         = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/mix448_true_caa_v4_spatial_layer_wdec_proj_mix_to_mix")
...
BETA   = 3.0  # Amplification factor on the W_dec[F] component of v_spatial
...
def compute_vectors(vsr_all):
    print("[STEP 1] Computing vectors from mix-448 last-token cache...", flush=True)
    ...
    for sf in SPATIAL_FEATURES:
        ...
        coeff = (v_sl * w_dec).sum()                    # v_spatial · W_dec[F]
        proj  = coeff * w_dec                            # component along W_dec[F]
        combined = v_sl + (BETA - 1.0) * proj            # amplify only that component
        v_sl_norm = v_sl.norm().clamp(min=1e-8)
        cos_sim   = (coeff / v_sl_norm).item()
        print(f"  [{key}] PROJ β={BETA}: coeff={coeff.item():+.4f}  cos(v_spatial, W_dec)={cos_sim:+.4f}  "
              f"norm: {v_sl_norm:.3f}→{combined.norm().item():.3f}", flush=True)
        v_proj[key] = {"layer": lyr, "vec": combined}

    return v_spatial, v_proj
...
def main():
    ...
    print("=" * 70)
    print("True CAA SPATIAL_LAYER + W_dec[F] PROJ+AMPLIFY β=3 — mix→mix (self-steering)")
    print("=" * 70, flush=True)
    ...
    v_spatial, v_proj = compute_vectors(vsr_all)
    gc.collect()
    ...
    for sf in SPATIAL_FEATURES:
        ...
        if key in v_proj:
            lyr = v_proj[key]["layer"]
            print(f"  [{key}] SPATIAL_LAYER_WDEC_PROJ steer (L{lyr}→L{lyr})...", flush=True)
            all_results = run_steer_sweep(
                f"{key}/PROJ", v_proj[key]["vec"], lyr,
                rel_vis, rel_labels, base_acc,
                f"{key}_spatial_layer_wdec_proj", all_results, results_path,
                model, processor, yes_ids, no_ids, device, vsr_all,
            )
    ...
    print(f"\n{'='*70}")
    print(f"True CAA SPATIAL_LAYER_WDEC_PROJ β={BETA} — mix→mix")
    print(f"{'='*70}")
    print(f"  {'Feature':<16} {'N':>5} {'Base':>7}  {'PROJ Δ':>10}")
    print("  " + "-"*42)
    for sf in SPATIAL_FEATURES:
        key = sf["key"]
        ...
        r = all_results.get(f"{key}_spatial_layer_wdec_proj", {})
        best = f"{max(v.get('delta',-999) for v in r.values()):+.2f}%" if r else "—"
        print(f"  {key:<16} {n:>5} {ba:>6.2f}%  {best:>10}")
