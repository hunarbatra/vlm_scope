#!/usr/bin/env python3
"""
Negative-α Rimsky sanity: if CAA direction is real "truth direction", then
injecting -α should HURT accuracy proportionally to how much +α helps.

Test on caa_all_layers_meanpool config (α=1 gave +4.42%, so α=-1 should give ~-4%).

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B caa_negative_alpha.py
"""
...
OUT_DIR = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/caa_negative_alpha")
...
def main():
    ...
    for alpha in ALPHAS:
        ...
        print(f"  [α={alpha}] NEG: {acc:.2f}%  Δ={d:+.2f}%  ({c}/{t})", flush=True)
