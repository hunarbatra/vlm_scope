#!/usr/bin/env python3
"""
Extended sweep for small relations needing more confidence:
  - alongside (N=55, L6 α=1.25 tentative override at +1.82pp)
  - next to (N=309, current oracle +1.62%, can we do better?)
  - opposite to (N=36, α=0.15 gave +5.56% vs oracle +2.78% — confirm?)

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_small_rels_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_small_rels_extended_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_small_rels_sweep")
...
TARGET_RELATIONS = [
    "alongside",   # L6 assigned, α=1.25 tentative (+1.82pp), N=55
    "next to",     # L9F7540 assigned, need broader sweep, N=309
    "opposite to", # L9F7540 assigned, α=0.15 tentative, N=36
]
...
def main():
    ...
    for rel, r in summary.items():
        ...
        print(f"\n{rel} (N={r.get('n',0)}, base={base:.2f}%):", flush=True)
        for k, v in sorted(feats.items(), key=lambda x: x[1].get('best_delta', -999), reverse=True):
            print(f"  {k}: oracle={v.get('oracle_delta',0):.2f}%, best={v['best_delta']:.2f}% @ α={v['best_alpha']}", flush=True)
