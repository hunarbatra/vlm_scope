#!/usr/bin/env python3
"""
OCR-Bench CAA steering eval: mix→pt, Recipe A (Middle) and Recipe D (BB+W(γ)).

Vector: mix-448 correct/incorrect on full OCR-Bench test set.
Eval on: full OCR-Bench test set (1000 samples).

Recipe A (MIDDLE):
  inject α·unit(v_L13) at L13 of pt-448 (text tokens only)

Recipe D (BB+W(γ)) — run once per feature F at layer lF:
  Backbone: inject α·unit(v_L) at ALL backbone layers {L17,L19,L20,L21}
  At lF additionally: inject γ·unit(W_dec[F])
  Total at lF: α·unit(v_lF) + γ·unit(W_dec[F])
  Sweep: α ∈ {0.5,1,2,5,10,20}, γ ∈ {1,3,10}

5 OCR-Bench features (home layers = backbone layers):
  L19/F10089 — Scene Text VQA  (spotlight: scene_text, regular_text)
  L17/F13602 — Scene Text      (spotlight: scene_text, regular_text)
  L20/F10687 — Document OCR    (spotlight: document, printed_text)
  L21/F9577  — Digit String    (spotlight: digit_string, number)
  L19/F14093 — Irregular Text  (spotlight: irregular_text, handwritten)

Usage:
  CUDA_VISIBLE_DEVICES=5 python3 steer_eval_ocr.py --gpu 0 --condition middle
  CUDA_VISIBLE_DEVICES=6 python3 steer_eval_ocr.py --gpu 0 --condition recipe_d --feature L19F10089
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis_ocr/steering_ocr")
...
FEATURES = [
    {"layer": 19, "feature": 10089, "label": "L19F10089",
     "category": "Scene Text VQA", "spotlight_types": ["scene_text", "regular_text"]},
    {"layer": 17, "feature": 13602, "label": "L17F13602",
     "category": "Scene Text",     "spotlight_types": ["scene_text", "regular_text"]},
    {"layer": 20, "feature": 10687, "label": "L20F10687",
     "category": "Document OCR",   "spotlight_types": ["document", "printed_text"]},
    {"layer": 21, "feature": 9577,  "label": "L21F9577",
     "category": "Digit String",   "spotlight_types": ["digit_string", "number"]},
    {"layer": 19, "feature": 14093, "label": "L19F14093",
     "category": "Irregular Text", "spotlight_types": ["irregular_text", "handwritten"]},
]
...
def eval_ocr(model, processor, tokenizer, samples, device, injector=None):
    ...
    for i, sample in enumerate(samples):
        ...
        t += 1
        if ok: c += 1
        cat = str(sample.get("question_type", sample.get("type", "unknown")))
        by_cat[cat]["t"] += 1
    ...
    return {"acc": 100*c/max(t,1), "correct": c, "total": t, "per_cat": per_cat}
...
def run_middle(model, processor, tokenizer, samples, device,
    ...
    for alpha in alphas:
        ...
        try:
            res = eval_ocr(model, processor, tokenizer, samples, device, inj)
        finally:
            inj.remove()
...
def print_summary(all_results, alphas, gammas, base_acc, results_path):
    print(f"\n{'='*72}")
    print(f"OCR-Bench CAA Steering  —  pt-448 base {base_acc:.2f}%")
    print(f"{'='*72}")
...
def main():
    ...
    v_mid = _load_vec("ocr", MIDDLE_LAYER)
    backbone_vecs = {}
    if args.condition in ("recipe_d", "both"):
        backbone_vecs = {l: _load_vec("ocr", l) for l in BACKBONE_LAYERS}
    ...
    if "baseline" not in all_results:
        ...
        base = eval_ocr(pt_model, pt_proc, pt_tok, samples, device)
