...
# Generic prompts for lexical artifact filtering — benign image-description
# prompts that avoid OCR-specific cues like "text", "read", "digit", "letter".
GENERIC_PROMPTS = [
    "Describe this image.",
...
# Spatial feature identification (match original find_ocr_features.py VSR example)
...
def _firing_worker(gpu_id, layer_indices):
    ...
    # VQA sample-level firing is cached in analysis_ocr/firing_vqa_sample (new) OR
    # computed fresh here. (We do NOT reuse analysis/firing_vqa_pertoken because that's
    # per-token, not sample-level; different Fisher denominator.)
    vqa_dir = ANALYSIS_DIR / "firing_vqa"
    vsr_dir = ANALYSIS_DIR / "firing_ocr"
    ...
    remaining_vsr = [l for l in layer_indices if not (vsr_dir / f"firing_ocr_layer_{l}.json").exists()]
    ...
    # --- Pass 2: OCR-Bench (sample-level firing) ---
    if remaining_vsr:
        print(f"[Firing GPU{gpu_id}] Loading OCR-Bench (test split)...")
        ocr = load_dataset("echo840/OCRBench", split="test")
        print(f"[Firing GPU{gpu_id}] OCR Pass: {len(ocr)} samples, layers: {remaining_vsr}", flush=True)
        ...
        for layer_idx in remaining_vsr:
            ...
            for si in range(len(ocr)):
                ...
                try:
                    ...
                    n_failed += 1
                    if n_failed <= 5:
                        print(f"  [GPU{gpu_id}] OCR err L{layer_idx} si={si}: {e}", flush=True)
                    continue

                if (si + 1) % 200 == 0 and gpu_id == 0:
                    print(f"  L{layer_idx} OCR: {si+1}/{len(ocr)} ({n_failed} failed)", flush=True)
            ...
            with open(vsr_dir / f"firing_ocr_layer_{layer_idx}.json", "w") as f:
                json.dump(layer_data, f)

            print(f"[Firing GPU{gpu_id}] OCR L{layer_idx}: {n_samples} samples ({n_failed} failed), "
                  f"features firing >50%: {(sample_fire_count > n_samples * 0.5).sum()}", flush=True)
...
# Matches original modal_vsr_firing.py find_ocr_features_vsr():
...
def step6_spatial():
    ...
    vqa_dir = ANALYSIS_DIR / "firing_vqa"
    vsr_dir = ANALYSIS_DIR / "firing_ocr"
    out_dir = ANALYSIS_DIR / "ocr"
    out_dir.mkdir(parents=True, exist_ok=True)

    rows = []
    for layer_idx in range(N_LAYERS):
        vqa_path = vqa_dir / f"firing_vqa_layer_{layer_idx}.json"
        vsr_path = vsr_dir / f"firing_ocr_layer_{layer_idx}.json"
    ...
    spatial.to_csv(out_dir / "ocr_features.csv", index=False)
    ...
    with open(out_dir / "ocr_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
...
def _lexical_worker(gpu_id, feature_assignments):
    ...
    Matches original probe_vqa_ocr_features.py:
    ...
    # For OCR, scan OCR-Bench directly (features should fire on OCR content)
    ocr = load_dataset("echo840/OCRBench", split="test")
    print(f"[Lexical GPU{gpu_id}] {len(ocr)} OCR-Bench samples available", flush=True)
    ...
    for layer_idx in sorted(layer_features.keys()):
        ...
        # --- Phase 1: Scan OCR-Bench samples to find top activators per feature ---
        feature_set = set(features)
        feature_cols = {f: [] for f in features}  # feature -> list of (max_act, ocr_idx)

        scan_count = min(scan_samples, len(ocr))
        print(f"[Lexical GPU{gpu_id}] L{layer_idx}: scanning {scan_count} OCR samples "
...
def step7_lexical(n_gpus=N_GPUS):
    ...
    csv_path = ANALYSIS_DIR / "ocr" / "ocr_features.csv"
    if not csv_path.exists():
...
def step8_intersection():
    ...
    # Load spatial
    spatial_path = ANALYSIS_DIR / "ocr" / "ocr_features.csv"
    spatial_by_layer = {}
    ...
    if final_features:
        ...
        df_final.to_csv(out_dir / "final_ocr_features.csv", index=False)
