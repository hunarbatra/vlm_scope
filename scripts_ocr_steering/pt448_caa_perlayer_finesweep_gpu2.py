#!/usr/bin/env python3
"""
Per-layer CAA fine-grained alpha sweep — GPU 2 variant.

Tests remaining 5 features not yet covered by GPU 4 script.
Uses extended alpha grid for caa_all_ml (around the α=7 peak discovered for L12/F2257).

Key insight from Exp 29: α=7.0 is optimal for L12/F2257 caa_all_ml (per-layer vectors).
This script tests if the same is true for other features.

Extended grids:
  caa_sae_down: [0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0]
  caa_all_ml:   [1.0, 2.0, 3.0, 5.0, 7.0, 10.0, 12.0, 15.0, 20.0, 30.0]

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_caa_perlayer_fine/

Usage:
    CUDA_VISIBLE_DEVICES=2 python3 pt448_caa_perlayer_finesweep_gpu2.py
"""
...
# Features that GPU 4 is processing slowly (high N, will take time)
# GPU 2 will process the remaining 5 features from the list that GPU 4 will get to later
FEATURES = [
    # GPU 4 will take long on L9/F387 (N=480), L15/F220 (N=515), L9/F7540 (N=35 fast),
    # L14/F10561 (N=93 fast), L13/F15219 (N=709), L6/F7539 (N=323), L11/F9639 (N=1101)
    # GPU 2 starts with the higher-N features to speed things up
...
ALPHA_SAE_DOWN = [0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0]
ALPHA_ALL_ML   = [1.0, 2.0, 3.0, 5.0, 7.0, 10.0, 12.0, 15.0, 20.0, 30.0]
DECAY_ML       = 0.7
...
def main():
    ...
    print(f"\n{'='*100}")
    print("Per-Layer CAA Fine Sweep (GPU 2) — Summary")
    print(f"{'='*100}")
