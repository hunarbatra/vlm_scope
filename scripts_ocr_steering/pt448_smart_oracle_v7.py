#!/usr/bin/env python3
"""
Smart Oracle v7 — Extended per-relation alpha overrides across L15/F220 and L6/F7539.

Changes from v6:
- L15/F220 "above":      α=0.8 → +5.87% (vs oracle α=0.7 → +4.69%, +1.18pp, N=341)
- L6/F7539 "at_back_of": α=1.75 → +3.19% (vs oracle α=1.5 → +2.13%, +1.06pp, N=94)
- L6/F7539 "below":      α=1.5 → +5.42% confirmed as oracle optimal (N=277) — no change
- All v6 overrides retained: L11 on_top_of→0.4, L11 surrounding→0.6,
  L9/F7540 in_middle_of→0.2, L9/F7540 on→0.3

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v7/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v7.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v7")
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v7_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v6 expected ~+4.30%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v7 — expanded per-relation alpha overrides (6 relations)", flush=True)
    ...
    print(f"\n[RESULT v7] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v6 expected ~+4.30%;  v5 ~+4.20%;  v4 ~+4.15%;  v3=+4.05%;  v2=+3.96%]", flush=True)
    ...
    res = {"version": "v7", "alpha_mode": "per_relation_alpha_6overrides",
