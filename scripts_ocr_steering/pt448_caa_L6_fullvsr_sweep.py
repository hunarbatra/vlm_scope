#!/usr/bin/env python3
"""
L6/F7539 full-VSR alpha sweep.

L6/F7539 is emerging as the most universally helpful feature in the per-relation
cross-steering matrix — best for "across from" (+14.89%), "alongside" (+16.36%),
"at the back of" (+3.19%), and strong positive on most other relations.

This script sweeps L6/F7539 at alphas [0.1, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0]
on the FULL VSR dataset to find the optimal alpha for full-dataset application.

The feature's own-relation optimal is α=1.5 (for "left of"/"right of"), but
cross-relation effects may prefer a different alpha. Finding the full-VSR optimum
is key for the oracle-free baseline: "if you had to pick ONE feature to inject
everywhere, what's the best result?"

Also tests L12/F2257 (best on "alongside" +14.55%, "ahead of" +15.38%) for
comparison as a second candidate for universal injection.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_caa_universal_sweep/

Usage:
    CUDA_VISIBLE_DEVICES=5 python3 pt448_caa_L6_fullvsr_sweep.py
"""
...
# Features to test with their own-relation optimal configs
FEATURES = [
    # (layer_idx, feature_idx, start_layer, alphas_to_test)
    (6,  7539, 1, [0.1, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0]),
    (12, 2257, 1, [0.1, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0]),
    (4,  14233, 0, [0.25, 0.5, 0.75, 1.0, 1.5, 2.0]),
    (15, 220,  15, [0.1, 0.25, 0.5, 0.75, 1.0]),
]
...
def main():
    ...
    result_path = OUT_DIR / "universal_sweep_results.json"
    ...
    # Alpha sweep for each feature on full VSR
...
def _print_summary(results):
    ...
    print(f"\n{'='*60}")
    print("Universal Injection Sweep Summary")
    print(f"{'='*60}")
