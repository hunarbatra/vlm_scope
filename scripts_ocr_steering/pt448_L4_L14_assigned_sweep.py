#!/usr/bin/env python3
"""
Alpha sweep for L4/F14233 and L14/F10561 on their oracle-assigned relations.

L4/F14233 oracle alpha=0.9, assigned: ahead of, behind
L14/F10561 oracle alpha=2.0, assigned: by, close to, connected to

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L4_L14_assigned_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_L4_L14_assigned_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L4_L14_assigned_sweep")
...
FEATURE_CONFIGS = {
    "L4_F14233":  {"layer": 4,  "feature": 14233, "start": 0,  "oracle_alpha": 0.9,
                   "alphas": [0.3, 0.5, 0.7, 0.9, 1.1, 1.5, 2.0, 2.5, 3.0]},
    "L14_F10561": {"layer": 14, "feature": 10561, "start": 0,  "oracle_alpha": 2.0,
                   "alphas": [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5]},
...
def main():
    ...
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':22s}  {'N':5s}  {'Base':7s}  {'L4(oracle)':10s}  {'L4(best)':8s}  {'L14(oracle)':11s}  {'L14(best)':9s}", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        base = r.get("base_acc", 0)
        feats = r.get("features", {})
        f4  = feats.get("L4_F14233", {})
        f14 = feats.get("L14_F10561", {})
        d4_ora  = f4.get("oracle_delta", 0) or 0
        d4_best = f4.get("best_delta", 0) or 0
        d14_ora  = f14.get("oracle_delta", 0) or 0
        d14_best = f14.get("best_delta", 0) or 0
        print(f"{rel:22s}  {r.get('n',0):5d}  {base:7.2f}%  {d4_ora:+10.2f}%  {d4_best:+8.2f}%  {d14_ora:+11.2f}%  {d14_best:+9.2f}%", flush=True)
