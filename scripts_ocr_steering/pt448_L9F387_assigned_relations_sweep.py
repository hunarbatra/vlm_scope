#!/usr/bin/env python3
"""
Fine alpha sweep for L9/F387 on all oracle-assigned relations.

L9/F387 oracle alpha is 0.4 (from subset sweeps).
This script tests all oracle-assigned relations for L9/F387 to find
per-relation optimal alphas.

Oracle-assigned relations for L9/F387:
  adjacent to, at the right side of, at the side of, attached to, far from

Also tests L12/F2257 and L15/F220 as cross-feature comparisons.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L9F387_assigned_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_L9F387_assigned_relations_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L9F387_assigned_sweep")
...
def main():
    ...
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':25s}  {'N':5s}  {'Base':7s}  {'L9F387(oracle)':14s}  {'L9F387(best)':12s}  {'L12_best':8s}  {'L15_best':8s}", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        base = r.get("base_acc", 0)
        feats = r.get("features", {})
        f9  = feats.get("L9_F387", {})
        f12 = feats.get("L12_F2257", {})
        f15 = feats.get("L15_F220", {})
        d9_ora  = f9.get("oracle_delta", 0) or 0
        d9_best = f9.get("best_delta", 0) or 0
        d12 = f12.get("best_delta", 0) or 0
        d15 = f15.get("best_delta", 0) or 0
        print(f"{rel:25s}  {r.get('n',0):5d}  {base:7.2f}%  {d9_ora:+14.2f}%  {d9_best:+12.2f}%  {d12:+8.2f}%  {d15:+8.2f}%", flush=True)
