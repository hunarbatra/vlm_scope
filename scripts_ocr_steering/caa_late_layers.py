#!/usr/bin/env python3
"""
CAA at ONLY late coherent layers {L11, L12, L13, L14, L15} — the cluster with
high cross-layer cosine similarity (>0.6) representing truth direction.

Drops early layers (L4, L6, L9) which have lower cosine with L13 and might add
noise.

Usage:
    CUDA_VISIBLE_DEVICES=X python3 -B caa_late_layers.py
"""
...
OUT_DIR = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/caa_late_layers")
...
TRAIN_END = 8777
LAYERS = [11, 12, 13, 14, 15]  # late coherent cluster only
...
def main():
    ...
    print(f"[INFO] LATE layers {LAYERS} meanpool CAA...", flush=True)
    vecs = {}
    for l in LAYERS:
        v = compute_caa(l, vsr_labels)
        if v is not None: vecs[l] = v / v.norm().clamp(min=1e-8)
    gc.collect()
    ...
    for alpha in ALPHAS:
        ...
        print(f"  [α={alpha}] LATE: {acc:.2f}%  Δ={d:+.2f}%  ({c}/{t})", flush=True)
