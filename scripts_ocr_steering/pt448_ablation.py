...
def _ablation_worker(gpu_id, feature_assignments, out_dir, vqa_max):
    ...
    for feat_i, (layer_idx, feature_idx, relations_str, odds_ratio) in enumerate(feature_assignments):
        ...
        print(f"[Ablation GPU{gpu_id}] L{layer_idx}/F{feature_idx}: "
              f"∆VSR={result['delta_vsr']:+.2f}% ({len(filtered_indices)} samp), "
...
def main():
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=str(CHECKPOINT_DIR),
                        help="SAE checkpoint directory")
    parser.add_argument("--analysis-dir", type=str, default=str(ANALYSIS_DIR),
                        help="Analysis directory (for spatial_pertoken/, cosines/, energy/)")
    ...
    or_map = {}
    spatial_path = ANALYSIS_DIR / "spatial_pertoken" / "spatial_features_pertoken.csv"
    ...
    # Load mix-448 delta_vsr for priority ordering (most negative = most interesting)
    mix_delta = {}
    mix_abl_csv = ANALYSIS_DIR / "ablation_per_relation_full" / "ablation_summary.csv"
    if mix_abl_csv.exists():
        with open(mix_abl_csv) as f:
            for row in csv.DictReader(f):
                mix_delta[(int(row["layer"]), int(row["feature"]))] = float(row["delta_vsr"])
    ...
    for layer, feature in features_raw:
        ...
        # Sort by mix-448 delta_vsr ascending (most negative drop first)
        mix_drop = mix_delta.get(key, 0.0)
        scored.append((layer, feature, rel_str, odds, mix_drop))

    scored.sort(key=lambda x: x[4])  # Most negative mix-448 drop first
    ...
    print(f"\nTop 10 by mix-448 drop:")
    for i, (l, f, r, o, s) in enumerate(scored[:10]):
        print(f"  {i+1}. L{l}/F{f}: mix_drop={s:+.2f}%, OR={o:.1f}, relations={r[:60]}")

    out_dir = Path(args.out_dir) if args.out_dir else ANALYSIS_DIR / "ablation_per_relation"

    # Distribute across GPUs (round-robin so each GPU gets a mix of large/small N)
    n_gpus = len(args.gpus)
    assignments = []
    for i, gpu_id in enumerate(args.gpus):
        worker_feats = features[i::n_gpus]
        if worker_feats:
