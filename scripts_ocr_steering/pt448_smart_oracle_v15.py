#!/usr/bin/env python3
"""
Smart Oracle v15 — Extended per-relation alpha overrides, 21 total (+1 new).

Changes from v14:
- L9/F7540 "opposite to": α=0.15 → +5.56% (vs oracle α=0.25 → +2.78%, +2.78pp, N=36)
  [Confirmed by three independent features all converging to very low alpha optimum]
- All v14 overrides retained

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v15/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v15.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v15")
...
RELATION_ALPHA_OVERRIDES = {
    ...
    "opposite to":      0.15, # +5.56% vs oracle +2.78%, +2.78pp, N=36 (NEW v15)
}
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v15_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v14 expected ~+5.00%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v15 — 21 per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v15] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v14 expected ~+5.00%;  v13 ~+4.90%;  v12 ~+4.82%;  v3=+4.05%]", flush=True)
    ...
    res = {"version": "v15", "alpha_mode": "per_relation_alpha_21overrides",
