#!/usr/bin/env python3
"""
Smart Oracle v6 — Per-relation alpha overrides for L11/F12278 and L9/F7540.

Changes from v5:
- L11/F12278: per-relation alpha overrides
  * "on top of":   α=0.4 → +5.35% (vs oracle α=0.45 → +4.95%, +0.40pp, N=505)
  * "surrounding": α=0.6 → +11.11% (vs oracle α=0.45 → +7.78%, +3.33pp, N=90) ← BIG GAIN
  * "touching":    α=0.45 → +6.01% (confirmed optimal, no change)
  * "under":       TBD (L11 latestart sweep still running — using α=0.45)
- L9/F7540: per-relation alpha overrides
  * "in the middle of": α=0.2 → +7.61% (from v5, confirmed)
  * "on":               α=0.3 → +1.37% (vs oracle α=0.25 → +0.34%, +1.03pp, N=585)
  * other L9/F7540 relations: α=0.25 confirmed optimal

All other configs same as v5.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v6/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_smart_oracle_v6.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_smart_oracle_v6")
...
# Default feature configs (per-feature optimal alphas, all confirmed from subset sweeps)
FEATURE_CONFIGS = {
    ...
    "L11_F12278": {"layer": 11, "feature": 12278, "start": 5,  "alpha": 0.45},  # default
    "L9_F387":    {"layer": 9,  "feature": 387,   "start": 1,  "alpha": 0.4},
    "L6_F7539":   {"layer": 6,  "feature": 7539,  "start": 1,  "alpha": 1.5},
    "L9_F7540":   {"layer": 9,  "feature": 7540,  "start": 9,  "alpha": 0.25},  # default
}

# Per-relation alpha overrides
# Applies when a relation's best alpha differs from the feature's default.
# Key is the RELATION string (must match parse_relation output exactly).
# Value is the ALPHA to use for that relation (overrides feature default).
RELATION_ALPHA_OVERRIDES = {
    # L11/F12278 relation-specific alphas
    "on top of":   0.4,  # L11: α=0.4 → +5.35% (vs default α=0.45 → +4.95%, +0.40pp, N=505)
    "surrounding": 0.6,  # L11: α=0.6 → +11.11% (vs default α=0.45 → +7.78%, +3.33pp, N=90)
    # "touching": 0.45,  # L11: confirmed optimal — no override needed
    # "under": TBD       # L11: latestart sweep still running

    # L9/F7540 relation-specific alphas
    "in the middle of": 0.2, # L9/F7540: α=0.2 → +7.61% (vs default α=0.25 → +5.43%, +2.18pp)
    "on":               0.3, # L9/F7540: α=0.3 → +1.37% (vs default α=0.25 → +0.34%, +1.03pp)
}
...
def main():
    ...
    result_path = OUT_DIR / "smart_oracle_v6_per_relation_alpha.json"
    if result_path.exists():
        print(f"[SKIP] {result_path} exists", flush=True)
        with open(result_path) as f:
            r = json.load(f)
        print(f"  Result: Δ={r['delta_acc']:+.2f}%  (v5 expected ~+4.2%)")
        return

    print(f"\n{'='*60}", flush=True)
    print(f"Smart Oracle v6 — expanded per-relation alpha overrides", flush=True)
    ...
    print(f"\n[RESULT v6] base={ba:.2f}%  smart={sa:.2f}%  Δ={sa-ba:+.2f}%  N={total}", flush=True)
    print(f"  [vs v5: expected ~+4.2%;  v4: expected ~+4.2%;  v3: +4.05%;  v2: +3.96%]", flush=True)
    ...
    res = {"version": "v6", "alpha_mode": "per_relation_alpha_expanded",
