...
For each unsafe candidate feature F from Step 6 (overall pass):
  1. Scan UP TO 500 VLSBench-unsafe samples → find top-5 most-activating samples
...
Output: analysis_safety/lexical/lexical_results_w<GPU>.json
...
UNSAFE_CSV    = ANALYSIS_DIR / "unsafe_pertoken" / "unsafe_features_pertoken.csv"
...
def load_unsafe_ids():
    ...
    for line in open(JUDGE_FILE):
        ...
        if r.get("judge_label") == "UNSAFE":
            unsafe.append(r["instruction_id"])
    return set(unsafe)
...
def _lexical_worker(gpu_id, feature_assignments):
    ...
    unsafe_set = load_unsafe_ids()
    keep_idx = [i for i in range(len(ds)) if str(ds[i]["instruction_id"]) in unsafe_set]
...
def main():
    ...
    if not UNSAFE_CSV.exists():
        print("[FATAL] unsafe_features_pertoken.csv missing. Run 31_fisher first."); return
    df = pd.read_csv(UNSAFE_CSV)
