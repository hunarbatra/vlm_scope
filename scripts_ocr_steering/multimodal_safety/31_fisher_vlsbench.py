...
Step 6 (safety variant) — Fisher exact test per feature:
  features firing significantly more on VLSBench-unsafe than on VQAv2 baseline.

Runs two passes:
  (a) Overall: VLSBench-unsafe (all 835) vs VQA 50K baseline  → "unsafe features"
  (b) Per-category: each of 6 categories vs VQA baseline       → category-specific features

Outputs:
  analysis_safety/unsafe_pertoken/unsafe_features_pertoken.csv
  analysis_safety/unsafe_pertoken/all_features_stats_pertoken.csv
  analysis_safety/unsafe_pertoken_by_cat/unsafe_features_<CAT>.csv   (per category)
...
UNSAFE_DIR = ROOT / "analysis_safety" / "firing_vlsbench_unsafe_pertoken"
CAT_DIR    = ROOT / "analysis_safety" / "firing_vlsbench_unsafe_by_cat"
OUT_ALL    = ROOT / "analysis_safety" / "unsafe_pertoken"
OUT_CAT    = ROOT / "analysis_safety" / "unsafe_pertoken_by_cat"
...
def main():
    ...
    print("\n[Pass a] Overall VLSBench-unsafe vs VQA")
    unsafe_layers = {}
    for l in range(N_LAYERS):
        p = UNSAFE_DIR / f"firing_vlsbench_layer_{l}.json"
    ...
    if not df.empty:
        ...
        unsafe.to_csv(OUT_ALL / "unsafe_features_pertoken.csv", index=False)
    ...
    # --- Pass (b): per-category ---
    print("\n[Pass b] Per-category VLSBench-unsafe vs VQA")
    # Discover categories from filenames
    cats = set()
    for p in CAT_DIR.glob("firing_vlsbench_*_layer_*.json"):
        # filename: firing_vlsbench_<CAT>_layer_<L>.json
        name = p.stem  # firing_vlsbench_Violent_layer_10
        parts = name.split("_layer_")[0]
        cat = parts[len("firing_vlsbench_"):]
        cats.add(cat)
    cats = sorted(cats)
    ...
    for cat in cats:
        layers_data = {}
        for l in range(N_LAYERS):
            p = CAT_DIR / f"firing_vlsbench_{cat}_layer_{l}.json"
        ...
        unsafe_cat.to_csv(OUT_CAT / f"unsafe_features_{cat}.csv", index=False)
