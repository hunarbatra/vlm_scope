#!/usr/bin/env python3
"""
Fine alpha sweep for L15/F220 on all oracle-assigned relations.

L15/F220 oracle alpha is 0.7 (from L15_subset_sweep on 'at the left side of').
This script tests all oracle-assigned relations for L15/F220 to find
per-relation optimal alphas.

Oracle-assigned relations for L15/F220:
  above, at_left_side_of, away_from, beside, contains, inside, over, part_of, within

Also tests L12/F2257 and L9/F7540 as cross-feature comparisons on each relation.

Output: /data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L15_assigned_sweep/
Usage: CUDA_VISIBLE_DEVICES=<gpu> python3 pt448_L15_assigned_relations_sweep.py
"""
...
OUT_DIR     = Path("/data1/vlm_scope_sae_mix448_textonly/analysis/pt448_L15_assigned_sweep")
...
# Relations currently assigned to L15/F220 in the oracle
TARGET_RELATIONS = [
    "above",
    "at the left side of",
    "away from",
...
# Features to test — L15 primary + cross-feature comparisons
FEATURE_CONFIGS = {
    "L15_F220":  {"layer": 15, "feature": 220,  "start": 15, "oracle_alpha": 0.7,
                  "alphas": [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.25, 1.5, 2.0]},
...
def main():
    ...
    # Summary table
    print("\n" + "=" * 80, flush=True)
    print(f"{'Relation':22s}  {'N':5s}  {'Base':7s}  {'L15(oracle)':11s}  {'L15(best)':9s}  {'L12_best':8s}  {'L9_best':7s}", flush=True)
    print("-" * 80, flush=True)
    for rel, r in summary.items():
        ...
        f9  = feats.get("L9_F7540", {})
        d15_ora = f15.get("oracle_delta", 0) or 0
        d15_best = f15.get("best_delta", 0) or 0
        d12 = f12.get("best_delta", 0) or 0
        d9  = f9.get("best_delta", 0) or 0
        print(f"{rel:22s}  {r.get('n',0):5d}  {base:7.2f}%  {d15_ora:+11.2f}%  {d15_best:+9.2f}%  {d12:+8.2f}%  {d9:+7.2f}%", flush=True)
