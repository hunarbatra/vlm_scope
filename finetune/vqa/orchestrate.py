...
def main():
    ...
    for chunk_start in range(args.resume_from_sample, args.n_training_samples, args.caching_chunk_size):
        ...
        # Step 2c: Clean up the chunk file to save memory
        chunk_file = RUN_DIR / "temp" / f"chunk_{chunk_start}_{chunk_end}.h5"
        if chunk_file.exists():
            try:
                chunk_file.unlink()
                print(f"[INFO] Cleaned up {chunk_file}")
            except Exception as e:
                print(f"[WARN] Could not delete {chunk_file}: {e}")

        print(f"[INFO] Completed chunk {chunk_count}: {chunk_start} → {chunk_end}")

    # Step 3: Clean up validation data
    print(f"\n[STEP 3] Cleaning up validation data...")
    val_chunk_file = RUN_DIR / "val_temp" / f"chunk_{val_start}_{val_end}.h5"
    if val_chunk_file.exists():
        try:
            val_chunk_file.unlink()
            (RUN_DIR / "val_temp").rmdir()
            print(f"[INFO] Cleaned up validation data")
        except Exception as e:
            print(f"[WARN] Could not delete validation data: {e}")
